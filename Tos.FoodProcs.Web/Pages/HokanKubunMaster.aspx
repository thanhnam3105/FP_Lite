<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true"
    CodeBehind="HokanKubunMaster.aspx.cs" Inherits="Tos.FoodProcs.Web.Pages.HokanKubunMaster" %>

<asp:Content ID="Content1" ContentPlaceHolderID="IncludeContent" runat="server">
    <script src="<%=ResolveUrl("~/Resources/pages/pagedata-hokankubunmaster." + System.Threading.Thread.CurrentThread.CurrentUICulture.TwoLetterISOLanguageName + ".js") %>"
    ...
    <script type="text/javascript">
        ...
        $(App.ui.page).on("ready", function () {
            ...
            */
            ...
            grid.jqGrid({
                colNames: [
                    pageLangText.cd_hokan_kbn.text + pageLangText.requiredMark.text,
                    pageLangText.nm_hokan_kbn.text + pageLangText.requiredMark.text,
                ...
                colModel: [
                    { name: 'cd_hokan_kbn', width: pageLangText.cd_hokan_kbn_width.number, hidden: false, editable: true, sorttype: "text" },
                    { name: 'nm_hokan_kbn', width: pageLangText.nm_hokan_kbn_width.number, hidden: false, editable: true, sorttype: "text" },
                ...
                loadComplete: function () {
                    ...
                    for (var i = 0; i < ids.length; i++) {
                        ...
                        if (grid.jqGrid('getCell', id, 'ts')) {
                            grid.jqGrid('setCell', id, 'cd_hokan_kbn', '', 'not-editable-cell');
                        }
                        // TODO：ここまで
                    }
                    // グリッドの先頭行選択
            ...
            var query = function () {
                var query = {
                    // TODO: 画面の仕様に応じて以下のサービスのエンドポイントを変更してください。
                    url: "../Services/FoodProcsService.svc/ma_kbn_hokan",
                    // TODO: ここまで
                    filter: createFilter(),
            ...
            var createFilter = function () {
                ...
                if (!App.isUndefOrNull(criteria.cd_hokan_kbn) && criteria.cd_hokan_kbn.length > 0) {
                    filters.push("cd_hokan_kbn eq " + criteria.cd_hokan_kbn);
                }
                if (!App.isUndefOrNull(criteria.nm_hokan_kbn) && criteria.nm_hokan_kbn.length > 0) {
                    filters.push("nm_hokan_kbn eq " + criteria.nm_hokan_kbn);
                }
                // TODO: ここまで
            ...
            var setCreatedChangeData = function (newRow) {
                // TODO: 画面の仕様に応じて以下の項目を変更してください。
                var changeData = {
                    "cd_hokan_kbn": newRow.cd_hokan_kbn,
                    "nm_hokan_kbn": newRow.nm_hokan_kbn,
            ...
            var setUpdatedChangeData = function (row) {
                // TODO: 画面の仕様に応じて以下の項目を変更してください。
                var changeData = {
                    "cd_hokan_kbn": row.cd_hokan_kbn,
                    "nm_hokan_kbn": row.nm_hokan_kbn,
                    ...
                    for (var i = 0; i < ret.length; i++) {
                        ...
                        if (ret[i].InvalidationName == "DuplicateKey") {
                            ...
                            for (var j = 0; j < ids.length; j++) {
                                ...
                                retValue = ret[i].Data.cd_hokan_kbn;
                            ...
                            for (p in changeSet.changeSet.deleted) {
                                ...
                                // TODO: 画面の仕様に応じて以下の値を変更します。
                                value = changeSet.changeSet.deleted[p].cd_hokan_kbn;
                                retValue = ret[i].Data.cd_hokan_kbn;
                                // TODO: ここまで
                ...
                if (!App.isUndefOrNull(ret.Updated) && ret.Updated.length > 0) {
                    for (var i = 0; i < ret.Updated.length; i++) {
                        for (p in changeSet.changeSet.updated) {
                            ...
                            retValue = ret.Updated[i].Requested.cd_hokan_kbn;
                            // TODO: ここまで
                            ...
                            if (value === retValue) {
                                ...
                                if (App.isUndefOrNull(upCurrent)) {
                                    ...
                                    current.cd_hokan_kbn = upCurrent.cd_hokan_kbn;
                                    current.nm_hokan_kbn = upCurrent.nm_hokan_kbn;
                ...
                if (!App.isUndefOrNull(ret.Deleted) && ret.Deleted.length > 0) {
                    for (var i = 0; i < ret.Deleted.length; i++) {
                        for (p in changeSet.changeSet.deleted) {
                            ...
                            // TODO: 画面の仕様に応じて以下の値を変更します。
                            value = changeSet.changeSet.deleted[p].cd_hokan_kbn;
                            retValue = ret.Deleted[i].Requested.cd_hokan_kbn;
                            // TODO: ここまで
            ...
            var saveData = function (e) {
                ...
                var saveUrl = "../api/HokanKubunMaster";
                App.ajax.webpost(
                    saveUrl, getPostData()
                ).done(function (result) {
                    // 検索前の状態に初期化
