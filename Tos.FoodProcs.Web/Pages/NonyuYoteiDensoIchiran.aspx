<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true"
    CodeBehind="NonyuYoteiDensoIchiran.aspx.cs" Inherits="Tos.FoodProcs.Web.Pages.NonyuYoteiDensoIchiran" %>

<asp:Content ID="Content1" ContentPlaceHolderID="IncludeContent" runat="server">
    <script src="<%=ResolveUrl("~/Resources/pages/pagedata-nonyuyoteidensoichiran." + System.Threading.Thread.CurrentThread.CurrentUICulture.TwoLetterISOLanguageName + ".js") %>"
    ...
    <script type="text/javascript">
        ...
        $(App.ui.page).on("ready", function () {
            ...
            */
            ...
            dateformat('dt_nonyu_start');
            dateformat('dt_nonyu_end');
            ...
            grid.jqGrid({
                ...
                colModel: [
                    ...
                    { name: 'dt_nonyu', width: 100, sorttype: "date",
                        formatter: "date",
                        formatoptions: {
                    ...
                    { name: 'no_nonyu', width: pageLangText.su_nonyu_width.number, editable: true, align: 'left', sorttype: "text" },
                    { name: 'cd_hinmei', width: 110, editable: false, align: 'left', sorttype: "text" },
                    { name: hinmeiName, width: 200, editable: false, align: 'left', sorttype: "text" },
                    { name: 'cd_torihiki', width: 110, editable: false, align: 'left', sorttype: "text" },
                    { name: 'nm_torihiki', width: 200, editable: false, align: 'left', sorttype: "text" },
                    { name: 'su_nonyu', width: 120, sorttype: "int", align: "right",
                        formatter: 'integer',
                        formatoptions: {
                            thousandsSeparator: ",", decimalPlaces: 0, defaultValue: "0"
                        }
                    },
                    { name: 'cd_tani', width: 100, editable: false, align: 'left', sorttype: "text" },
                    { name: 'nm_tani', width: 100, editable: false, align: 'left', sorttype: "text" },
                    { name: 'kbn_nyuko', width: 100, sorttype: "text", align: "left",
                        formatter: setNyukoKubun
                    }
                ],
                datatype: "local",
                shrinkToFit: false,
            ...
            // 入庫区分の名称を設定
            function setNyukoKubun(value, options, rowObj) {
                var returnVal = "";
                var nyukoData = pageLangText.nyukoKunbunId.data;
                for (var i = 0; i < nyukoData.length; i++) {
                    if (value == nyukoData[i].id) {
                        returnVal = nyukoData[i].name;
                        break;
                    }
                }
                return returnVal;
            ...
            var query = function () {
                ...
                var flgDenso = pageLangText.falseFlg.text,
                    flgNonyu = pageLangText.falseFlg.text;
                ...
                if (criteria.chk_nonyu == "on") {
                    flgNonyu = pageLangText.trueFlg.text;
                }

                var query = {
                    url: "../api/NonyuDensoIchiran",
                    ...
                    dt_nonyu_from: App.data.getDateTimeStringForQueryNoUtc(criteria.dt_nonyu_start),
                    dt_nonyu_to: App.data.getDateTimeStringForQueryNoUtc(criteria.dt_nonyu_end),
                    cd_hinmei: encodeURIComponent(criteria.cd_hinmei),
                    no_nonyu: encodeURIComponent(criteria.noNonyu),
                    chk_denso: flgDenso,
                    chk_nonyu: flgNonyu,
                    //put_char: putChar,
            ...
            var checkDateSearch = function () {
                ...
                // 製品ロットNo以外の項目が1つは入力されているかチェック
                if ((App.isUndefOrNull(criteria.chk_denso)) && (App.isUndefOrNull(criteria.chk_nonyu))
                  && (App.isUndefOrNull(criteria.cd_hinmei)) && (App.isUndefOrNull(criteria.noNonyu))
                ) {
                ...
                if (criteria.chk_denso != null) {
                    ...
                    var DateTo = new Date($("#dt_denso_end").datepicker("getDate"));
                    if (DateFrom > DateTo) {
                    ...
                    chk_dt_denso = pageLangText.trueFlg.text;
                }
                else {
                    chk_dt_denso = pageLangText.falseFlg.text;
                }
                // 検索条件/製造日チェック
                if (criteria.chk_nonyu != null) {
                    var DateFrom = new Date($("#dt_nonyu_start").datepicker("getDate"));
                    var DateTo = new Date($("#dt_nonyu_end").datepicker("getDate"));
                    if (DateFrom > DateTo) {
                        App.ui.page.notifyAlert.message(App.str.format(pageLangText.inputDateError.text, pageLangText.dateNonyuEn.text, pageLangText.dateNonyuSt.text), $("#dt_nonyu_start")).show();
                        return false;
                    }
                    chk_dt_nonyu = pageLangText.trueFlg.text;
                }
                else {
                    chk_dt_nonyu = pageLangText.falseFlg.text;
                }
                return true;
            };
            ...
            var isValidHinmeiCode = function (hinmeiCode) {
                ...
                var queryHin = {
                    ...
                    filter: "cd_hinmei eq '" + criteria.cd_hinmei
            ...
            */
            ...
            $("#cd_seihin").on("dblclick", showHinmeiDialog);
            /// <summary> 品名一覧ボタンクリック時のイベント処理を行います。</summary>
            $("#seihin-category-button").on("click", showHinmeiDialog);
        });
    </script>
</asp:Content>
            ...
            <ul class="item-list">
                    ...
                    <table width="90%">
                        <tr>
                            <td width="5%"  height="20" style="padding-bottom: 5px;">
                                ...
                                <span data-app-text="to" style="text-align:center; width:30px;"></span>
                        ...
                        <tr>
                            <td width="5%"  height="20" style="padding-bottom: 5px;">
                                <input type="checkbox" name="chk_nonyu" id="chk_nonyu" checked="checked"/>
                                <span class="item-label" data-app-text="dateNonyuSt"></span>
                                <input type="text" name="dt_nonyu_start" id="dt_nonyu_start" />
                                <span data-app-text="to" style="text-align:center; width:30px;"></span>
                                <span class="item-label" data-app-text="dateNonyuEn"></span>
                                <input type="text" name="dt_nonyu_end" id="dt_nonyu_end" />
                            </td>
                        </tr>
                    </table>
                </li>
                <li>
                    <label>
						...
						<input type="text" class="changeCheck" name="cd_hinmei" id="cd_seihin" maxlength="14" />
                ...
                <li>
                    <label>
                        <span class="item-label" data-app-text="noNonyu" style="padding-left:26px;"></span>
                        <input type="text" class="changeCheck" name="noNonyu" id="noNonyu"  maxlength="14"/>
                    </label>
                </li>
                <!-- TODO: ここまで -->
            </ul>
