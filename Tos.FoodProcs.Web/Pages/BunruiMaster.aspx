<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true"
    CodeBehind="BunruiMaster.aspx.cs" Inherits="Tos.FoodProcs.Web.Pages.BunruiMaster" %>

<asp:Content ID="Content1" ContentPlaceHolderID="IncludeContent" runat="server">
    <script src="<%=ResolveUrl("~/Resources/pages/pagedata-bunruimaster." + System.Threading.Thread.CurrentThread.CurrentUICulture.TwoLetterISOLanguageName + ".js") %>"
        ...
        $(App.ui.page).on("ready", function () {
            ...
            */
            ...
            grid.jqGrid({
                ...
                colNames: [
                    pageLangText.hinKubun.text + pageLangText.requiredMark.text, pageLangText.bunruiCode.text + pageLangText.requiredMark.text,
                    pageLangText.bunruiName.text + pageLangText.requiredMark.text, pageLangText.mishiyoFlag.text,
                    pageLangText.createCode.text, pageLangText.createDate.text, pageLangText.ts.text
                ],
                // TODO：ここまで
                // TODO：画面の仕様に応じて以下の列モデルの定義を変更してください。
                colModel: [
                    { name: 'kbn_hin', width: 80, editable: false, sortable: false, hidden: true, hidedlg: true, align: 'left' },
                    { name: 'cd_bunrui', width: pageLangText.cd_bunrui_width.number, editable: true, toggle: true, sortable: true, hidden: false, hidedlg: false, align: 'left' },
                    { name: 'nm_bunrui', width: pageLangText.nm_bunrui_width.number, editable: true, sorttype: "text", align: 'left' },
                    ...
                    { name: 'dt_create', width: 0, hidden: true, hidedlg: true,
                        formatter: "date",
                        formatoptions: {
                            srcformat: pageLangText.dateTimeNewFormat.text, newformat: pageLangText.dateTimeNewFormat.text
                        }
                    },
                ...
                loadComplete: function () {
                    ...
                    for (var i = 0; i < ids.length; i++) {
                        ...
                        if (grid.jqGrid('getCell', id, 'ts')) {
                            grid.jqGrid('setCell', id, 'cd_bunrui', '', 'not-editable-cell');
                        }
                    }
                },
            ...
            App.deferred.parallel({
                ...
                + " or kbn_hin eq " + pageLangText.shikakariHinKbn.text
                ...
                // 検索用ドロップダウンの設定
                App.ui.appendOptions($(".search-criteria [name='hinKubun']"), "kbn_hin", "nm_kbn_hin", hinKubun, false);
                // TODO: ここまで
            ...
            var bindData = function (result) {
                ...
                if (querySetting.count > querySetting.top) {
                    App.ui.page.notifyAlert.message(
                        App.str.format(pageLangText.overSearchCount.text, querySetting.count, querySetting.top)
                    ).show();
                }
                // グリッドの表示件数を更新
            ...
            var setAddData = function () {
                ...
                var getHinKubun = function () {
                    if (!App.isUndefOrNull(searchCondition)) {
                        return searchCondition.hinKubun;
                    }
                    else { return ""; }
                };
                var addData = {
            ...
            var setCreatedChangeData = function (newRow) {
                ...
                var changeData = {
                    ...
                    "cd_bunrui": newRow.cd_bunrui,
                    "nm_bunrui": newRow.nm_bunrui,
            ...
            var setUpdatedChangeData = function (row) {
                ...
                var changeData = {
                    "kbn_hin": row.kbn_hin,
                    "cd_bunrui": row.cd_bunrui,
                    "nm_bunrui": row.nm_bunrui,
                    ...
                    for (var i = 0; i < ret.length; i++) {
                        ...
                        if (ret[i].InvalidationName === "NotExsists") {
                            ...
                            for (var j = 0; j < ids.length; j++) {
                                ...
                                if (value === retValue) {
                                    ...
                                    // 対象セルの背景変更
                                    grid.setCell(ids[j], firstCol, ret[i].Data.cd_bunrui, { background: '#ff6666' });
                                    // TODO: ここまで
                                }
                            }
                        }
                        if (ret[i].InvalidationName === "DuplicateKey") {
                            ...
                            for (p in changeSet.changeSet.deleted) {
                                ...
                                // TODO: 画面の仕様に応じて以下の値を変更します。
                                value = changeSet.changeSet.deleted[p].cd_bunrui;
                                retValue = ret[i].Data.cd_bunrui;
                                // TODO: ここまで
                ...
                if (!App.isUndefOrNull(ret.Updated) && ret.Updated.length > 0) {
                    for (var i = 0; i < ret.Updated.length; i++) {
                        for (p in changeSet.changeSet.updated) {
                            ...
                            retValue = ret.Updated[i].Requested.cd_bunrui;
                            // TODO: ここまで
                            ...
                            if (value === retValue) {
                                ...
                                if (App.isUndefOrNull(ret.Updated[i].Current)) {
                                    ...
                                    current.kbn_hin = ret.Updated[i].Current.kbn_hin;
                                    current.cd_bunrui = ret.Updated[i].Current.cd_bunrui;
                                    current.nm_bunrui = ret.Updated[i].Current.nm_bunrui;
                ...
                if (!App.isUndefOrNull(ret.Deleted) && ret.Deleted.length > 0) {
                    for (var i = 0; i < ret.Deleted.length; i++) {
                        for (p in changeSet.changeSet.deleted) {
                            ...
                            // TODO: 画面の仕様に応じて以下の値を変更します。
                            value = changeSet.changeSet.deleted[p].cd_bunrui;
                            retValue = ret.Deleted[i].Requested.cd_bunrui;
                            // TODO: ここまで
    ...
    <div class="content-part search-criteria">
        ...
        <div class="part-body">
            <ul class="item-list">
                ...
                <li>
                    <label>
                        <span class="item-label" data-app-text="hinKubun"></span>
