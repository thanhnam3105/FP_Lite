<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true"
    CodeBehind="LocationMaster.aspx.cs" Inherits="Tos.FoodProcs.Web.Pages.LocationMaster" %>

<asp:Content ID="Content1" ContentPlaceHolderID="IncludeContent" runat="server">
    <script src="<%=ResolveUrl("~/Resources/pages/pagedata-locationmaster." + System.Threading.Thread.CurrentThread.CurrentUICulture.TwoLetterISOLanguageName + ".js") %>"
    ...
    <script type="text/javascript">
        ...
        $(App.ui.page).on("ready", function () {
            ...
            */
            ...
            //$(".search-criteria [name='dt_hiduke']").datepicker({ dateFormat: pageLangText.dateFormat.text });
            // TODO：ここまで
            ...
            grid.jqGrid({
                ...
                colNames: [
                    pageLangText.locationCode.text + pageLangText.requiredMark.text
                    , pageLangText.locationName.text + pageLangText.requiredMark.text
                    , pageLangText.mishiyoFlg.text
                    , pageLangText.torokuDate.text
                    , pageLangText.torokuCode.text
                    , pageLangText.koshinDate.text
                    , pageLangText.koshinCode.text
                    , pageLangText.ts.text
                ],
                // TODO：ここまで
                // TODO：画面の仕様に応じて以下の列モデルの定義を変更してください。
                colModel: [
                    { name: 'cd_location', width: 100, editable: true, hidden: false },
                    { name: 'nm_location', width: 400, editable: true, sorttype: "text" },
                    { name: 'flg_mishiyo', width: 70, editable: true, edittype: 'checkbox',
                ...
                loadComplete: function () {
                    ...
                    for (var i = 0; i < ids.length; i++) {
                        ...
                        if (grid.jqGrid('getCell', id, 'ts')) {
                            grid.jqGrid('setCell', id, 'cd_location', '', 'not-editable-cell');
                        }
                    }
                    // グリッドの先頭行選択
            ...
            var bindData = function (result) {
                querySetting.skip = querySetting.skip + result.Items.length;
                querySetting.count = parseInt(result.Count);
                ...
                displayCount();
                // データバインド
                var currentData = grid.getGridParam("data").concat(result.Items);
            ...
            var setCreatedChangeData = function (newRow) {
                // TODO: 画面の仕様に応じて以下の項目を変更してください。
                var changeData = {
                    "cd_location": newRow.cd_location,
                    "nm_location": newRow.nm_location,
            ...
            var setUpdatedChangeData = function (row) {
                // TODO: 画面の仕様に応じて以下の項目を変更してください。
                var changeData = {
                    "cd_location": row.cd_location,
                    "nm_location": row.nm_location,
                    ...
                    for (var i = 0; i < ret.length; i++) {
                        ...
                        if (ret[i].InvalidationName == "NotExsists") {
                            ...
                            for (var j = 0; j < ids.length; j++) {
                                ...
                                retValue = ret[i].Data.cd_location;
                                // TODO: ここまで
                                ...
                                if (value === retValue) {
                                    ...
                                    // 対象セルの背景変更
                                    grid.setCell(ids[j], firstCol, ret[i].Data.cd_location, { background: '#ff6666' });
                                    // TODO: ここまで
                                }
                            }
                        }
                        if (ret[i].InvalidationName == "DuplicateKey") {
                            ...
                            for (p in changeSet.changeSet.deleted) {
                                ...
                                // TODO: 画面の仕様に応じて以下の値を変更します。
                                value = changeSet.changeSet.deleted[p].cd_location;
                                retValue = ret[i].Data.cd_location;
                                // TODO: ここまで
                ...
                if (!App.isUndefOrNull(ret.Updated) && ret.Updated.length > 0) {
                    for (var i = 0; i < ret.Updated.length; i++) {
                        for (p in changeSet.changeSet.updated) {
                            ...
                            retValue = ret.Updated[i].Requested.cd_location;
                            // TODO: ここまで
                            ...
                            if (value === retValue) {
                                ...
                                if (App.isUndefOrNull(upCurrent)) {
                                    ...
                                    current.cd_location = upCurrent.cd_location;
                                    current.nm_location = upCurrent.nm_location;
                                    current.flg_mishiyo = upCurrent.flg_mishiyo;
                                    current.dt_create = App.data.getDate(upCurrent.dt_create);
                ...
                if (!App.isUndefOrNull(ret.Deleted) && ret.Deleted.length > 0) {
                    for (var i = 0; i < ret.Deleted.length; i++) {
                        for (p in changeSet.changeSet.deleted) {
                            ...
                            // TODO: 画面の仕様に応じて以下の値を変更します。
                            value = changeSet.changeSet.deleted[p].cd_location;
                            retValue = ret.Deleted[i].Requested.cd_location;
                            // TODO: ここまで
            ...
            var saveData = function (e) {
                ...
                var saveUrl = "../api/LocationMaster";
                App.ajax.webpost(
                    saveUrl, getPostData()
                ).done(function (result) {
                    // 検索前の状態に初期化
            ...
            /// <summary>保存ダイアログのはいボタンクリック時のイベント処理を行います。</summary>
            $(".save-confirm-dialog .dlg-yes-button").on("click", saveData);
            /// <summary>保存ダイアログのいいえボタンクリック時のイベント処理を行います。</summary>
...
<asp:Content ID="Content4" ContentPlaceHolderID="FooterContent" runat="server">
    ...
    <div class="command" style="left: 9px;">
