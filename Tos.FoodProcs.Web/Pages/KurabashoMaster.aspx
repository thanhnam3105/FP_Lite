<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true"
    CodeBehind="KurabashoMaster.aspx.cs" Inherits="Tos.FoodProcs.Web.Pages.KurabashoMaster" %>

<asp:Content ID="Content1" ContentPlaceHolderID="IncludeContent" runat="server">
    <script src="<%=ResolveUrl("~/Resources/pages/pagedata-kurabashomaster." + System.Threading.Thread.CurrentThread.CurrentUICulture.TwoLetterISOLanguageName + ".js") %>"
    ...
    <script type="text/javascript">
        ...
        $(App.ui.page).on("ready", function () {
            ...
            */
            ...
            grid.jqGrid({
                colNames: [
                    pageLangText.cd_kura.text + pageLangText.requiredMark.text
                    , pageLangText.nm_kura.text + pageLangText.requiredMark.text
                    , pageLangText.flg_mishiyo.text
                    , pageLangText.ts.text
                ],
                colModel: [
                    { name: 'cd_kura', width: pageLangText.cd_kura_width.number, editable: true, sorttype: "text" },
                    { name: 'nm_kura', width: pageLangText.nm_kura_width.number, editable: true, sorttype: "text" },
                ...
                loadComplete: function () {
                    ...
                    for (var i = 0; i < ids.length; i++) {
                        ...
                        if (grid.jqGrid('getCell', id, 'ts')) {
                            // 既存行のコードは編集不可にする
                            grid.jqGrid('setCell', id, 'cd_kura', '', 'not-editable-cell');
                        }
                    }
            ...
            var query = function () {
                var query = {
                    url: "../Services/FoodProcsService.svc/ma_kura",
                    //filter: createFilter(),
                    orderby: "cd_kura",
            ...
            var setCreatedChangeData = function (newRow) {
                var changeData = {
                    "cd_kura": newRow.cd_kura,
                    "nm_kura": newRow.nm_kura,
                    "cd_create": App.ui.page.user.Code,
                    "cd_update": App.ui.page.user.Code,
                    ...
                    for (var i = 0; i < ret.length; i++) {
                        if (ret[i].InvalidationName === "DuplicateKey") {
                            ...
                            for (var j = 0; j < ids.length; j++) {
                                value = grid.getCell(ids[j], kuraCodeCol);
                                retValue = ret[i].Data.cd_kura;

                                if (value == retValue) {
                                    unique = ids[j] + "_" + kuraCodeCol;
                                    // エラー行を追加
                                    errRows.push(ids[j]);
                                    ...
                                    // 対象セルの背景変更
                                    grid.setCell(ids[j], kuraCodeCol, retValue, { background: '#ff6666' });
                                }
                            }
                        }
                        else {
                            ...
                            for (p in changeSet.changeSet.deleted) {
                                ...
                                value = changeSet.changeSet.deleted[p].cd_kura;
                                retValue = ret[i].Data.cd_kura;
                                // TODO: ここまで
                    ...
                    for (var i = 0; i < ret.Updated.length; i++) {
                        for (p in changeSet.changeSet.updated) {
                            ...
                            if (App.isUndefOrNull(upCurrent)) {
                                ...
                                current.nm_kura = upCurrent.nm_kura;
            ...
            var saveData = function (e) {
                ...
                var saveUrl = "../api/KurabashoMaster";
                App.ajax.webpost(
                    saveUrl, getPostData()
                ).done(function (result) {
                    // 検索前の状態に初期化
            ...
            var clearErrBgcorror = function (errIds) {
                for (var i = 0; i < errIds.length; i++) {
                    var id = errIds[i];
                    // 対象セルの背景リセット
                    grid.setCell(id, kuraCodeCol, '', { background: 'none' });
                }
            };
