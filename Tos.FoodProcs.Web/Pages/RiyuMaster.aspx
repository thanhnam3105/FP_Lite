<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true"
    CodeBehind="RiyuMaster.aspx.cs" Inherits="Tos.FoodProcs.Web.Pages.RiyuMaster" %>

<asp:Content ID="Content1" ContentPlaceHolderID="IncludeContent" runat="server">
    <script src="<%=ResolveUrl("~/Resources/pages/pagedata-riyumaster." + System.Threading.Thread.CurrentThread.CurrentUICulture.TwoLetterISOLanguageName + ".js") %>"
    ...
    <script type="text/javascript">
        ...
        $(App.ui.page).on("ready", function () {
            ...
            */
            ...
            grid.jqGrid({
                colNames: [
                    pageLangText.cd_riyu.text + pageLangText.requiredMark.text
                    , pageLangText.nm_riyu.text + pageLangText.requiredMark.text
                    , pageLangText.kbn_bunrui_riyu.text
                    , pageLangText.ts.text
                    , pageLangText.flg_kinshi.text
                    , pageLangText.flg_denso.text
                ],
                colModel: [
                    { name: 'cd_riyu', width: 100, editable: true, sorttype: "text" },
                    { name: 'nm_riyu', width: 400, editable: true, sorttype: "text" },
                    { name: 'kbn_bunrui_riyu', width: 0, hidden: true, hidedlg: true },
                    { name: 'ts', width: 0, hidden: true, hidedlg: true },
                    { name: 'flg_kinshi', width: 70, hidden: true, hidedlg: true, editable: false, edittype: 'checkbox',
                        editoptions: { value: "1:0" }, formatter: 'checkbox', formatoptions: { disabled: true },
                        align: 'center'
                    },
                    { name: 'flg_denso', width: 70, editable: false, edittype: 'checkbox',
                        editoptions: { value: "1:0" }, formatter: 'checkbox', formatoptions: { disabled: true },
                        align: 'center'
                    }
                ],
                ...
                loadComplete: function () {
                    ...
                    for (var i = 0; i < ids.length; i++) {
                        ...
                        if (grid.jqGrid('getCell', id, 'flg_kinshi') == true) {
                            // 禁止フラグ.禁止の行は理由名を変更不可にする
            ...
            /// <summary>伝送表示切替区分の取得</summary>
            var getKbnDenso = function () {
                App.deferred.parallel({
                    kbnDenso: App.ajax.webgetSync("../Services/FoodProcsService.svc/cn_kino_sentaku()?$filter=kbn_kino eq " + pageLangText.kinoDensoHyojiKbn.number)
                }).done(function (result) {
                    kbnDenso = result.successes.kbnDenso.d;
                    if (kbnDenso.length > 0) {
                        denso = kbnDenso[0].kbn_kino_naiyo;
                    } else {
                        denso = "";
                    }
            ...
            if (denso != pageLangText.kbnDensoHyoji.number) {
                grid.jqGrid('hideCol', "flg_denso");
                grid.setColProp('flg_denso', { hidedlg: true });
            }


            // pageLangTextを用いた事前データロード
            riyuBunrui = pageLangText.riyuBunruiKubunId.data;
            // 検索用ドロップダウンの設定
            App.ui.appendOptions($(".search-criteria [name='kbn_bunrui_riyu']"), "id", "name", riyuBunrui, false);
            ...
            var query = function () {
                var query = {
                    url: "../Services/FoodProcsService.svc/ma_riyu",
                    filter: createFilter(),
                    orderby: "cd_riyu",
            ...
            var createFilter = function () {
                ...
                filters.push("kbn_bunrui_riyu eq " + criteria.kbn_bunrui_riyu);
            ...
            var bindData = function (result) {
                ...
                if (resultCount > querySetting.top) {
                    ...
                    App.ui.page.notifyInfo.message(MS0011).show();
                }
                else {
                    querySetting.skip = resultCount;
                }
            ...
            var setAddData = function () {
                var addData = {
                    "cd_riyu": "",
                    "nm_riyu": "",
                    "kbn_bunrui_riyu": $(".search-criteria [name='kbn_bunrui_riyu']").val(),
            ...
            var setCreatedChangeData = function (newRow) {
                var changeData = {
                    ...
                    "kbn_bunrui_riyu": newRow.kbn_bunrui_riyu,
                    "cd_create": App.ui.page.user.Code,
                    "cd_update": App.ui.page.user.Code,
                    "ts": newRow.ts,
                    "flg_kinshi": newRow.flg_kinshi,
            ...
            var setUpdatedChangeData = function (row) {
                var changeData = {
                    "cd_riyu": row.cd_riyu,
                    "nm_riyu": row.nm_riyu,
                    "kbn_bunrui_riyu": row.kbn_bunrui_riyu,
                    "cd_update": App.ui.page.user.Code,
                    "ts": row.ts,
                    "flg_kinshi": "",
                    "flg_denso": ""
            ...
            var deleteData = function (e) {
                ...
                //禁止フラグ.禁止のデータは削除不可
                if (grid.jqGrid('getCell', selectedRowId, 'flg_kinshi') == true) {
                    App.ui.page.notifyAlert.message(pageLangText.notDel.text).show();
                    return;
                }
                // セル編集内容の保存
                grid.saveCell(currentRow, currentCol);
            ...
            var changeCondition = function () {
                ...
                if (criteria.kbn_bunrui_riyu != searchCondition.kbn_bunrui_riyu) {
                    return true;
                }
                return false;
            };
                    ...
                    for (var i = 0; i < ret.length; i++) {
                        if (ret[i].InvalidationName === "DuplicateKey") {
                            ...
                            for (var j = 0; j < ids.length; j++) {
                                value = grid.getCell(ids[j], riyuCodeCol);
                                retValue = ret[i].Data.cd_riyu;

                                if (value == retValue) {
                                    unique = ids[j] + "_" + riyuCodeCol;
                                    // エラー行を追加
                                    errRows.push(ids[j]);
                                    ...
                                    // 対象セルの背景変更
                                    grid.setCell(ids[j], riyuCodeCol, retValue, { background: '#ff6666' });
                                }
                            }
                        }
                        else {
                            ...
                            for (p in changeSet.changeSet.deleted) {
                                ...
                                value = parseInt(changeSet.changeSet.deleted[p].cd_riyu, 10)
                                retValue = ret[i].Data.cd_riyu;
                    ...
                    for (var i = 0; i < ret.Updated.length; i++) {
                        for (p in changeSet.changeSet.updated) {
                            ...
                            if (App.isUndefOrNull(upCurrent)) {
                                ...
                                current.kbn_bunrui_riyu = upCurrent.kbn_bunrui_riyu;
                                current.cd_create = upCurrent.cd_create;
            ...
            var clearErrBgcorror = function (errIds) {
                for (var i = 0; i < errIds.length; i++) {
                    var id = errIds[i];
                    // 対象セルの背景リセット
                    grid.setCell(id, riyuCodeCol, '', { background: 'none' });
                }
            };
    ...
    <div class="content-part search-criteria">
        ...
        <div class="part-body">
            <ul class="item-list">
                ...
                <li>
                    <label>
                        <span class="item-label" data-app-text="kbn_bunrui_riyu" style="width:100px;"></span>
                        <select name="kbn_bunrui_riyu" id="id_kbn_bunrui_riyu" style="width:200px;">
                        </select>
                    </label>
                </li>
                <!-- TODO: ここまで -->
