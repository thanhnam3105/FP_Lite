...
namespace Tos.FoodProcs.Web.Controllers
{
	...
	public class KojoMasterController : ApiController
	{
		// POST api/ma_kojo
		/// <summary>
		...
		// [Authorize(Roles="")]
		public HttpResponseMessage Post([FromBody]ChangeSet<ma_kojo> value)
		{
			...
			DuplicateSet<ma_kojo> duplicates = new DuplicateSet<ma_kojo>();
			// TODO: ここまで
			// TODO：整合性チェックエラーの結果を格納するInvalidationSetを定義します。
			InvalidationSet<ma_kojo> invalidations = new InvalidationSet<ma_kojo>();
			// TODO: ここまで
			...
			if (value.Updated != null)
			{
				foreach (var updated in value.Updated)
				{
					...
					if (!String.IsNullOrEmpty(validationMessage))
					{
                        ...
                        //invalidations.Add(new Invalidation<ma_kojo>(validationMessage, updated, Resources.NotExsists));
                        // TODO: ここまで
						continue;
					}

					// TODO: 既存エンティティを取得します。
					ma_kojo current = GetSingleEntity(context, updated.cd_kaisha, updated.cd_kojo);
					// TODO: ここまで
					...
					{
						duplicates.Updated.Add(new Duplicate<ma_kojo>(updated, current));
						continue;
					}

                    // 既存データをコピー(画面からは入力の無い値)
                    updated.kbn_kowake_futai = current.kbn_kowake_futai;
                    updated.nm_kaisha = current.nm_kaisha;
                    updated.dt_create = current.dt_create;
                    updated.cd_riyu = current.cd_riyu;
                    updated.cd_genka_center = current.cd_genka_center;
                    updated.cd_soko = current.cd_soko;
                    updated.su_keta_shosuten = current.su_keta_shosuten;
                    // 更新日時の設定
                    updated.dt_update = DateTime.UtcNow;

                    // TODO: エンティティを更新します。
                    context.ma_kojo.ApplyOriginalValues(updated);
                    context.ma_kojo.ApplyCurrentValues(updated);
                    // TODO: ここまで
				}
			}
			...
			if (invalidations.Count > 0)
			{
				...
				return Request.CreateResponse<InvalidationSet<ma_kojo>>(HttpStatusCode.BadRequest, invalidations);
				// TODO: ここまで
			}
			...
			if (duplicates.Updated.Count > 0)
			{
                ...
                return Request.CreateResponse<DuplicateSet<ma_kojo>>(HttpStatusCode.Conflict, duplicates);
				// TODO: ここまで
			}
		...
		// TODO：既存エンティティを取得します。
		private ma_kojo GetSingleEntity(FoodProcsEntities context, string cd_kaisha, string cd_kojo)
		{
            var result = context.ma_kojo.SingleOrDefault(ma => ma.cd_kaisha == cd_kaisha && ma.cd_kojo == cd_kojo);
		...
		// TODO: エンティティに対する整合性チェック (マスタ存在チェックなど) を行います。
        private string ValidateKey(FoodProcsEntities context, ma_kojo kojo)
        {
            var master = (from m in context.ma_kojo
                          where m.cd_kaisha == kojo.cd_kaisha
                          && m.cd_kojo == kojo.cd_kojo
