...
namespace Tos.FoodProcs.Web.Controllers
{
	...
	{
        // odataを利用してのリターン
        public PageResult<ma_location> Get(ODataQueryOptions<ma_location> options)
        {
            FoodProcsEntities context = new FoodProcsEntities();
            IQueryable results = options.ApplyTo(context.ma_location.AsQueryable());

            return new PageResult<ma_location>(
                results as IEnumerable<ma_location>,
		...
		// [Authorize(Roles="")]
        public HttpResponseMessage Post([FromBody]ChangeSet<ma_location> value)
		{
            ...
            DuplicateSet<ma_location> duplicates = new DuplicateSet<ma_location>();
			// TODO: ここまで
			// TODO：整合性チェックエラーの結果を格納するInvalidationSetを定義します。
            InvalidationSet<ma_location> invalidations = new InvalidationSet<ma_location>();
			// TODO: ここまで
			...
			if (value.Created != null)
			{
                ...
                foreach (var created in value.Created)
				{
                    ...
                    {
                        invalidations.Add(new Invalidation<ma_location>(validationMessage, created, Resources.DuplicateKey));
					...
					// TODO: エンティティを追加します。
                    context.AddToma_location(created);
					// TODO: ここまで
				}
			}
			...
			if (value.Updated != null && !flgSkip)
			{
				foreach (var updated in value.Updated)
				{
					...
					// TODO: 既存エンティティを取得します。
                    ma_location current = GetSingleEntity(context, updated.cd_location);
					// TODO: ここまで
					...
					{
                        duplicates.Updated.Add(new Duplicate<ma_location>(updated, current));
						continue;
					}
                    updated.dt_create = current.dt_create;

                    // TODO: エンティティを更新します。
                    context.ma_location.ApplyOriginalValues(updated);
                    context.ma_location.ApplyCurrentValues(updated);
                    // TODO: ここまで
				}
			}
			...
			if (value.Deleted != null && !flgSkip)
			{
				foreach (var deleted in value.Deleted)
				{
                    ...
                    {
                        // TODO: バリデーションエラーの発生した列名を指定してInvalidationSetを追加します。
                        invalidations.Add(new Invalidation<ma_location>(validationMessage, deleted, Resources.UnDeletableRecord));
                        // TODO: ここまで
                        continue;
                    }

					// TODO: 既存エンティティを取得します。
                    ma_location current = GetSingleEntity(context, deleted.cd_location);
					// TODO: ここまで
					...
					{
                        duplicates.Deleted.Add(new Duplicate<ma_location>(deleted, current));
						continue;
					}
			...
			if (invalidations.Count > 0)
			{
                ...
                return Request.CreateResponse<InvalidationSet<ma_location>>(HttpStatusCode.BadRequest, invalidations);
				// TODO: ここまで
			}
			...
			if (duplicates.Created.Count > 0 || duplicates.Updated.Count > 0 || duplicates.Deleted.Count > 0)
			{
                ...
                return Request.CreateResponse<DuplicateSet<ma_location>>(HttpStatusCode.Conflict, duplicates);
				// TODO: ここまで
			}
		...
		// TODO：既存エンティティを取得します。
        private ma_location GetSingleEntity(FoodProcsEntities context, string cd_location)
		{
            var result = context.ma_location.SingleOrDefault(ma => ma.cd_location == cd_location);
		...
		// TODO: エンティティに対する整合性チェック (マスタ存在チェックなど) を行います。
        private string ValidateKey(FoodProcsEntities context, ma_location location)
        {
            var master = (from m in context.ma_location
                          where m.cd_location == location.cd_location
        ...
        private String ValidateMasterTable(FoodProcsEntities context, ma_location ma)
        {
            /////// ラインマスタ(ma_line) ///////
            var maHinmei = (from m in context.ma_hinmei
                          where m.cd_location == ma.cd_location
                                 select m).FirstOrDefault();
            // 存在する場合、メッセージを返却
            if (maHinmei != null)
            {
                return getValidateErrorMassage(Resources.HinmeiMaster, ma.cd_location);
            }
        ...
        private static void AddKey(List<dynamic> keys, ma_location entity)
        {
            // 比較対象のキー値をセット
            dynamic key = new JObject();
            key.cd_location = entity.cd_location;
            keys.Add(key);
        }
        ...
        private String ValidateDuplicatKey(FoodProcsEntities context, ma_location ma,
        ...
        /// <returns>チェック結果</returns>
        private static bool ContainsKey(List<dynamic> keys, ma_location entity)
        {
            return keys.Find(k => k.cd_location == entity.cd_location) != null;
        }
	}
}