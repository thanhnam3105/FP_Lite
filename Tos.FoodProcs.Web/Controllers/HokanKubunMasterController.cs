...
namespace Tos.FoodProcs.Web.Controllers
{
    ...
    public class HokanKubunMasterController : ApiController
    {
        // POST api/ma_kbn_hokan
        ...
        // [Authorize(Roles="")]
        public HttpResponseMessage Post([FromBody]ChangeSet<ma_kbn_hokan> value)
        {
            ...
            DuplicateSet<ma_kbn_hokan> duplicates = new DuplicateSet<ma_kbn_hokan>();
            // 整合性チェックエラーの結果を格納するInvalidationSetを定義します。
            InvalidationSet<ma_kbn_hokan> invalidations = new InvalidationSet<ma_kbn_hokan>();
            ...
            if (value.Created != null)
            {
                ...
                foreach (var created in value.Created)
                {
                    ...
                    if (!String.IsNullOrEmpty(validationMessage))
                    {
                        ...
                        invalidations.Add(new Invalidation<ma_kbn_hokan>(validationMessage, created, Resources.DuplicateKey));
                    ...
                    // エンティティを追加します。
                    context.AddToma_kbn_hokan(created);
                }
            }
            ...
            if (value.Updated != null && !flgSkip)
            {
                ...
                {
                    // 既存エンティティを取得します。
                    ma_kbn_hokan current = GetSingleEntity(context, updated.cd_hokan_kbn);

                    // 既存行が無い、もしくはタイムスタンプの値が違う場合は、
                    // 他のユーザーに削除もしくは更新されたと判定し、競合データとして扱います。
                    if (current == null || !CompareByteArray(current.ts, updated.ts))
                    {
                        duplicates.Updated.Add(new Duplicate<ma_kbn_hokan>(updated, current));
                        continue;
                    }
                    ...
                    // エンティティを更新します。
                    context.ma_kbn_hokan.ApplyOriginalValues(updated);
                    context.ma_kbn_hokan.ApplyCurrentValues(updated);
                }
            }
            ...
            if (value.Deleted != null && !flgSkip)
            {
                ...
                {
                    // 既存エンティティを取得します。
                    ma_kbn_hokan current = GetSingleEntity(context, deleted.cd_hokan_kbn);

                    // 既存行が無い、もしくはタイムスタンプの値が違う場合は、
                    // 他のユーザーに削除もしくは更新されたと判定し、競合データとして扱います。
                    if (current == null || !CompareByteArray(current.ts, deleted.ts))
                    {
                        duplicates.Deleted.Add(new Duplicate<ma_kbn_hokan>(deleted, current));
                        continue;
                    }
            ...
            if (invalidations.Count > 0)
            {
                ...
                return Request.CreateResponse<InvalidationSet<ma_kbn_hokan>>(HttpStatusCode.BadRequest, invalidations);
            }
            ...
            if (duplicates.Created.Count > 0 || duplicates.Updated.Count > 0 || duplicates.Deleted.Count > 0)
            {
                ...
                return Request.CreateResponse<DuplicateSet<ma_kbn_hokan>>(HttpStatusCode.Conflict, duplicates);
            }
        ...
        /// <param name="cd_hokan_kbn">保管区分コード</param>
        /// <returns>既存エンティティ</returns>
        private ma_kbn_hokan GetSingleEntity(FoodProcsEntities context, string cd_hokan_kbn)
        {
            var result = context.ma_kbn_hokan.SingleOrDefault(ma => ma.cd_hokan_kbn == cd_hokan_kbn);
        ...
        private static void AddKey(List<dynamic> keys, ma_kbn_hokan entity)
        {
            // 比較対象のキー値をセット
            dynamic key = new JObject();
            key.cd_hokan_kbn = entity.cd_hokan_kbn;
            keys.Add(key);
        }
        ...
        private String ValidateDuplicatKey(FoodProcsEntities context, ma_kbn_hokan ma,
            List<dynamic> keys, List<dynamic> delKeys)
        {
            String errMsg = Resources.MS0027;    // エラーメッセージ

            // 既存データをチェック
            var master = (from m in context.ma_kbn_hokan
                          where m.cd_hokan_kbn == ma.cd_hokan_kbn
        ...
        /// <returns>チェック結果</returns>
        private static bool ContainsKey(List<dynamic> keys, ma_kbn_hokan entity)
        {
            return keys.Find(k => k.cd_hokan_kbn == entity.cd_hokan_kbn) != null;
        }
    }
}