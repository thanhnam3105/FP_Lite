...
namespace Tos.FoodProcs.Web.Controllers
{
    ...
    public class HyojunFundoJokagenchiMasterController : ApiController
	{
		...
		// [Authorize(Roles="")]
		public HttpResponseMessage Post([FromBody]ChangeSet<ma_fundo> value)
		{
            ...
            DuplicateSet<ma_fundo> duplicates = new DuplicateSet<ma_fundo>();
			// 整合性チェックエラーの結果を格納するInvalidationSetを定義します。
			InvalidationSet<ma_fundo> invalidations = new InvalidationSet<ma_fundo>();
			...
			if (value.Created != null)
			{
                ...
                {
                    // 重複チェック：分銅コードが重複していないこと
                    validationMessage = ValidateKey(context, created, createKeys, deleteKeys);
                    if (!String.IsNullOrEmpty(validationMessage))
                    {
                        invalidations.Add(new Invalidation<ma_fundo>(validationMessage, created, Resources.DuplicateKey));
			...
			if (value.Updated != null && !flgSkip)
			{
				...
				{
                    // 既存エンティティを取得します。
                    ma_fundo current = GetSingleEntity(context, updated.cd_fundo);

                    // 既存行が無い、もしくはタイムスタンプの値が違う場合は、
                    // 他のユーザーに削除もしくは更新されたと判定し、競合データとして扱います。
                    if (current == null || !CompareByteArray(current.ts, updated.ts))
                    { 
                        duplicates.Updated.Add(new Duplicate<ma_fundo>(updated, current));
                        continue;
                    }
                    ...
                    // エンティティを更新します。
                    context.ma_fundo.ApplyOriginalValues(updated);
                    context.ma_fundo.ApplyCurrentValues(updated);
				}
			}
            ...
            if (value.Deleted != null && !flgSkip)
            {
                foreach (var deleted in value.Deleted)
                {
                    ...
                    validationMessage = Validate_ma_hakari(context, deleted);
					if (!String.IsNullOrEmpty(validationMessage))
					{
                        // TODO: バリデーションエラーの発生した列名を指定してInvalidationSetを追加します。
                        invalidations.Add(new Invalidation<ma_fundo>(validationMessage, deleted, Resources.UnDeletableRecord));
                        continue;
                        // TODO: ここまで
					}

                    // 既存エンティティを取得します。
                    ma_fundo current = GetSingleEntity(context, deleted.cd_fundo);

                    // 既存行が無い、もしくはタイムスタンプの値が違う場合は、
                    // 他のユーザーに削除もしくは更新されたと判定し、競合データとして扱います。
                    if (current == null || !CompareByteArray(current.ts, deleted.ts))
                    {
                        duplicates.Deleted.Add(new Duplicate<ma_fundo>(deleted, current));
                        continue;
                    }
			...
			if (invalidations.Count > 0)
			{
				...
				return Request.CreateResponse<InvalidationSet<ma_fundo>>(HttpStatusCode.BadRequest, invalidations);
			}
            ...
            if (duplicates.Created.Count > 0 || duplicates.Updated.Count > 0 || duplicates.Deleted.Count > 0)
            {
                ...
                return Request.CreateResponse<DuplicateSet<ma_fundo>>(HttpStatusCode.Conflict, duplicates);
            }
        ...
        private ma_fundo GetSingleEntity(FoodProcsEntities context, String key)
		{
            var result = context.ma_fundo.SingleOrDefault(ma => ma.cd_fundo == key);
		...
		/// <param name="fundo">1レコード分の分銅マスタ情報</param>
		/// <param name="keys">更新対象のキーリスト</param>
		/// <param name="delKeys">削除対象のキーリスト</param>
		/// <returns>チェック結果：エラーの場合、エラーメッセージを返却</returns>
        private String ValidateKey(FoodProcsEntities context, ma_fundo fundo, List<dynamic> keys, List<dynamic> delKeys)
        {
            String errMsg = String.Format(Resources.MS0045, Resources.Code);    // エラーメッセージ

            var master = (from m in context.ma_fundo
                          where m.cd_fundo == fundo.cd_fundo
                          select m).FirstOrDefault();

            if (master != null)
            {
                // キーリストにない かつ 削除対象に存在する場合はエラーとしない
                if (!ContainsKey(keys, fundo) && ContainsKey(delKeys, fundo))
                {
                    return String.Empty;
                }
                return errMsg;
            }
            else if (ContainsKey(keys, fundo))
            {
                return errMsg;
            }
        ...
        /// <summary>
		/// 削除時の整合性チェック：秤マスタで使用されている場合、エラー
		/// </summary>
		/// <param name="context">エンティティ情報</param>
		/// <param name="ma">対象のエンティティ</param>
		/// <returns>チェック結果</returns>
        private String Validate_ma_hakari(FoodProcsEntities context, ma_fundo ma)
        {
            var master = (from m in context.ma_hakari
                          where m.cd_fundo == ma.cd_fundo
                          select m).FirstOrDefault();

            // 存在する場合、メッセージを返します
            string errMsg = getValidateErrorMassage(Resources.HakariMaster, ma.cd_fundo);
        ...
        /// <summary>
        /// 対象のキー(分銅コード)を持つエンティティの存在チェックを行います。
        /// </summary>
        /// <param name="keys">キーリスト</param>
        /// <param name="entity">チェック対象の追加レコード</param>
        /// <returns>チェック結果</returns>
        private static bool ContainsKey(List<dynamic> keys, ma_fundo entity)
        {
            return keys.Find(k => k.cd_fundo == entity.cd_fundo) != null;
        }
        ...
        private static void AddKey(List<dynamic> keys, ma_fundo entity)
        {
            // 比較対象のキー値をセット
            dynamic key = new JObject();
            key.cd_fundo = entity.cd_fundo;
            keys.Add(key);
        }
	}
}