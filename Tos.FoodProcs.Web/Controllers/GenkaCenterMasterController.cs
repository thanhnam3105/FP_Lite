...
namespace Tos.FoodProcs.Web.Controllers
{
    ...
    public class GenkaCenterMasterController : ApiController
	{
        // POST api/ZanTran
		/// <summary>
		...
		// [Authorize(Roles="")]
		public HttpResponseMessage Post([FromBody]ChangeSet<ma_genka_center> value)
		{
            ...
            DuplicateSet<ma_genka_center> duplicates = new DuplicateSet<ma_genka_center>();
			// TODO: ここまで
			// TODO：整合性チェックエラーの結果を格納するInvalidationSetを定義します。
            InvalidationSet<ma_genka_center> invalidations = new InvalidationSet<ma_genka_center>();
			// TODO: ここまで
			...
			if (value.Created != null)
			{
                ...
                foreach (var created in value.Created)
				{
                    ...
                    {
                        invalidations.Add(new Invalidation<ma_genka_center>(validationMessage, created, Resources.DuplicateKey));
					...
					// TODO: エンティティを追加します。
                    context.AddToma_genka_center(created);
					// TODO: ここまで
				}
			}
			...
			if (value.Updated != null && !flgSkip)
			{
				foreach (var updated in value.Updated)
				{
					...
					// TODO: 既存エンティティを取得します。
                    ma_genka_center current = GetSingleEntity(context, updated.cd_genka_center);
					// TODO: ここまで
					...
					{
                        duplicates.Updated.Add(new Duplicate<ma_genka_center>(updated, current));
						continue;
					}
                    updated.dt_create = current.dt_create;

                    // TODO: エンティティを更新します。
                    context.ma_genka_center.ApplyOriginalValues(updated);
                    context.ma_genka_center.ApplyCurrentValues(updated);
                    // TODO: ここまで
				}
			}
			...
			if (value.Deleted != null && !flgSkip)
			{
				foreach (var deleted in value.Deleted)
				{
                    ...
                    {
                        // TODO: バリデーションエラーの発生した列名を指定してInvalidationSetを追加します。
                        invalidations.Add(new Invalidation<ma_genka_center>(validationMessage, deleted, Resources.UnDeletableRecord));
                        // TODO: ここまで
                        continue;
                    }

					// TODO: 既存エンティティを取得します。
                    ma_genka_center current = GetSingleEntity(context, deleted.cd_genka_center);
					// TODO: ここまで
					...
					{
                        duplicates.Deleted.Add(new Duplicate<ma_genka_center>(deleted, current));
						continue;
					}
			...
			if (invalidations.Count > 0)
			{
                ...
                return Request.CreateResponse<InvalidationSet<ma_genka_center>>(HttpStatusCode.BadRequest, invalidations);
				// TODO: ここまで
			}
			...
			if (duplicates.Created.Count > 0 || duplicates.Updated.Count > 0 || duplicates.Deleted.Count > 0)
			{
                ...
                return Request.CreateResponse<DuplicateSet<ma_genka_center>>(HttpStatusCode.Conflict, duplicates);
				// TODO: ここまで
			}
		...
		// TODO：既存エンティティを取得します。
        private ma_genka_center GetSingleEntity(FoodProcsEntities context, string cd_genka_center)
		{
            var result = context.ma_genka_center.SingleOrDefault(ma => ma.cd_genka_center == cd_genka_center);
		...
		// TODO: エンティティに対する整合性チェック (マスタ存在チェックなど) を行います。
        private string ValidateKey(FoodProcsEntities context, ma_genka_center genka)
        {
            var master = (from m in context.ma_genka_center
                          where m.cd_genka_center == genka.cd_genka_center
        ...
        private static void AddKey(List<dynamic> keys, ma_genka_center entity)
        {
            // 比較対象のキー値をセット
            dynamic key = new JObject();
            key.cd_genka_center = entity.cd_genka_center;
            keys.Add(key);
        }
        ...
        private String ValidateDuplicatKey(FoodProcsEntities context, ma_genka_center ma,
            List<dynamic> keys, List<dynamic> delKeys)
        {
            ...
            // 既存データをチェック
            var master = (from m in context.ma_genka_center
                          where m.cd_genka_center == ma.cd_genka_center
                          select m).FirstOrDefault();
        ...
        /// <returns>チェック結果</returns>
        private static bool ContainsKey(List<dynamic> keys, ma_genka_center entity)
        {
            return keys.Find(k => k.cd_genka_center == entity.cd_genka_center) != null;
        }
	}
}
