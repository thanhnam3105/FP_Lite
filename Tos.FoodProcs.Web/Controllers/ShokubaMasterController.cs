...
namespace Tos.FoodProcs.Web.Controllers
{
	...
	public class ShokubaMasterController : ApiController
	{
        // odataを利用してのリターン
        public PageResult<ma_shokuba> Get(ODataQueryOptions<ma_shokuba> options)
        {
            FoodProcsEntities context = new FoodProcsEntities();
            IQueryable results = options.ApplyTo(context.ma_shokuba.AsQueryable());

            return new PageResult<ma_shokuba>(
                results as IEnumerable<ma_shokuba>,
		...
		// [Authorize(Roles="")]
		public HttpResponseMessage Post([FromBody]ChangeSet<ma_shokuba> value)
		{
			...
			DuplicateSet<ma_shokuba> duplicates = new DuplicateSet<ma_shokuba>();
			// TODO: ここまで
			// TODO：整合性チェックエラーの結果を格納するInvalidationSetを定義します。
            InvalidationSet<ma_shokuba> invalidations = new InvalidationSet<ma_shokuba>();
			// TODO: ここまで
			...
			if (value.Created != null)
			{
				...
				{
                    // 重複チェック：職場コードが重複していないこと
                    validationMessage = ValidateDuplicatKey(context, created, createKeys, deleteKeys);
                    if (!String.IsNullOrEmpty(validationMessage))
                    {
                        invalidations.Add(new Invalidation<ma_shokuba>(validationMessage, created, Resources.DuplicateKey));
					...
					// TODO: エンティティを追加します。
					context.AddToma_shokuba(created);
					// TODO: ここまで
				}
			}
			...
			if (value.Updated != null && !flgSkip)
			{
				foreach (var updated in value.Updated)
				{
					...
					// TODO: 既存エンティティを取得します。
					ma_shokuba current = GetSingleEntity(context, updated.cd_shokuba);
					// TODO: ここまで
					...
					{
                        duplicates.Updated.Add(new Duplicate<ma_shokuba>(updated, current));
						continue;
					}
                    updated.dt_create = current.dt_create;

                    // TODO: エンティティを更新します。
                    context.ma_shokuba.ApplyOriginalValues(updated);
                    context.ma_shokuba.ApplyCurrentValues(updated);
                    // TODO: ここまで
				}
			}
			...
			if (value.Deleted != null && !flgSkip)
			{
				foreach (var deleted in value.Deleted)
				{
                    ...
                    {
                        // TODO: バリデーションエラーの発生した列名を指定してInvalidationSetを追加します。
                        invalidations.Add(new Invalidation<ma_shokuba>(validationMessage, deleted, Resources.UnDeletableRecord));
                        // TODO: ここまで
                        continue;
                    }

					// TODO: 既存エンティティを取得します。
                    ma_shokuba current = GetSingleEntity(context, deleted.cd_shokuba);
					// TODO: ここまで
					...
					{
                        duplicates.Deleted.Add(new Duplicate<ma_shokuba>(deleted, current));
						continue;
					}
			...
			if (invalidations.Count > 0)
			{
                ...
                return Request.CreateResponse<InvalidationSet<ma_shokuba>>(HttpStatusCode.BadRequest, invalidations);
				// TODO: ここまで
			}
			...
			if (duplicates.Created.Count > 0 || duplicates.Updated.Count > 0 || duplicates.Deleted.Count > 0)
			{
                ...
                return Request.CreateResponse<DuplicateSet<ma_shokuba>>(HttpStatusCode.Conflict, duplicates);
				// TODO: ここまで
			}
		...
		// TODO：既存エンティティを取得します。
        private ma_shokuba GetSingleEntity(FoodProcsEntities context, string cd_shokuba)
		{
            var result = context.ma_shokuba.SingleOrDefault(ma => ma.cd_shokuba == cd_shokuba);
		...
		// TODO: エンティティに対する整合性チェック (マスタ存在チェックなど) を行います。
        private string ValidateKey(FoodProcsEntities context, ma_shokuba shokuba) { 
            var master = (from m in context.ma_shokuba
                          where m.cd_shokuba == shokuba.cd_shokuba
        ...
        private String ValidateMasterTable(FoodProcsEntities context, ma_shokuba ma)
        {
            /////// ラインマスタ(ma_line) ///////
            var maLine = (from m in context.ma_line
                                 where m.cd_shokuba == ma.cd_shokuba
                                 select m).FirstOrDefault();
            // 存在する場合、メッセージを返却
            if (maLine != null)
            {
                return getValidateErrorMassage(Resources.LineMaster, ma.cd_shokuba);
            }
        ...
        private static void AddKey(List<dynamic> keys, ma_shokuba entity)
        {
            // 比較対象のキー値をセット
            dynamic key = new JObject();
            key.cd_shokuba = entity.cd_shokuba;
            keys.Add(key);
        }
        ...
        private String ValidateDuplicatKey(FoodProcsEntities context, ma_shokuba ma,
        ...
        /// <summary>
        /// 対象のキー(職場コード)を持つエンティティの存在チェックを行います。
        /// </summary>
        /// <param name="keys">キーリスト</param>
        /// <param name="entity">チェック対象の追加レコード</param>
        /// <returns>チェック結果</returns>
        private static bool ContainsKey(List<dynamic> keys, ma_shokuba entity)
        {
            return keys.Find(k => k.cd_shokuba == entity.cd_shokuba) != null;
        }
	}
}