...
namespace Tos.FoodProcs.Web.Controllers
{
	...
	public class FutaiMasterController : ApiController
	{
		// POST api/ma_futai
		/// <summary>
		...
		// [Authorize(Roles="")]
		public HttpResponseMessage Post([FromBody]ChangeSet<ma_futai> value)
		{
			...
			DuplicateSet<ma_futai> duplicates = new DuplicateSet<ma_futai>();
			// 整合性チェックエラーの結果を格納するInvalidationSetを定義します。
			InvalidationSet<ma_futai> invalidations = new InvalidationSet<ma_futai>();
			...
			if (value.Created != null)
			{
                ...
                foreach (var created in value.Created)
				{
					...
					if (!String.IsNullOrEmpty(validationMessage))
					{
                        ...
                        invalidations.Add(new Invalidation<ma_futai>(validationMessage, created, Resources.DuplicateKey));
			...
			if (value.Updated != null && !flgSkip)
			{
				...
				{
					// 既存エンティティを取得します。
					ma_futai current = GetSingleEntity(context, updated.cd_futai);

					// 既存行が無い、もしくはタイムスタンプの値が違う場合は、
					// 他のユーザーに削除もしくは更新されたと判定し、競合データとして扱います。
					if (current == null || !CompareByteArray(current.ts, updated.ts))
					{
						duplicates.Updated.Add(new Duplicate<ma_futai>(updated, current));
						continue;
					}

                    // エンティティを更新します。
                    updated.dt_update = DateTime.UtcNow;
                    context.ma_futai.ApplyOriginalValues(updated);
                    context.ma_futai.ApplyCurrentValues(updated);
				}
			}
			...
			if (value.Deleted != null && !flgSkip)
			{
				foreach (var deleted in value.Deleted)
				{
                    ...
                    validationMessage = Validate_ma_futai_kettei(context, deleted);
					if (!String.IsNullOrEmpty(validationMessage))
					{
                        // バリデーションエラーの発生した列名を指定してInvalidationSetを追加します。
                        invalidations.Add(new Invalidation<ma_futai>(validationMessage, deleted, Resources.UnDeletableRecord));
                        continue;
					}

                    // 既存エンティティを取得します。
					ma_futai current = GetSingleEntity(context, deleted.cd_futai);

                    // 既存行が無い、もしくはタイムスタンプの値が違う場合は、
                    // 他のユーザーに削除もしくは更新されたと判定し、競合データとして扱います。
                    if (current == null || !CompareByteArray(current.ts, deleted.ts))
					{
						duplicates.Deleted.Add(new Duplicate<ma_futai>(deleted, current));
						continue;
					}
			...
			if (invalidations.Count > 0)
			{
				...
				return Request.CreateResponse<InvalidationSet<ma_futai>>(HttpStatusCode.BadRequest, invalidations);
			}
			...
			if (duplicates.Created.Count > 0 || duplicates.Updated.Count > 0 || duplicates.Deleted.Count > 0)
			{
                ...
                return Request.CreateResponse<DuplicateSet<ma_futai>>(HttpStatusCode.Conflict, duplicates);
			}
		...
		private ma_futai GetSingleEntity(FoodProcsEntities context, string cd_futai)
		{
            var result = context.ma_futai.SingleOrDefault(ma => ma.cd_futai == cd_futai);

			return result;
		}

        /// <summary>
		/// 削除時の整合性チェック：風袋決定マスタで使用されている場合、エラー
		/// </summary>
		/// <param name="context">エンティティ情報</param>
		/// <param name="ma">対象のエンティティ</param>
		/// <returns>チェック結果</returns>
        private String Validate_ma_futai_kettei(FoodProcsEntities context, ma_futai ma)
        {
            var master = (from m in context.ma_futai_kettei
                          where m.cd_futai == ma.cd_futai
                          && m.flg_mishiyo == ActionConst.FlagFalse
                          select m).FirstOrDefault();

            // 存在する場合、メッセージを返します
            string errMsg = getValidateErrorMassage(Resources.FutaiKetteiMaster, ma.cd_futai);
        ...
        private static void AddKey(List<dynamic> keys, ma_futai entity)
        {
            // 比較対象のキー値をセット
            dynamic key = new JObject();
            key.cd_futai = entity.cd_futai;
            keys.Add(key);
        }
        ...
        private String ValidateDuplicatKey(FoodProcsEntities context, ma_futai ma,
            List<dynamic> keys, List<dynamic> delKeys)
        {
            String errMsg = Resources.MS0027;    // エラーメッセージ

            // 既存データをチェック
            var master = (from m in context.ma_futai
                          where m.cd_futai == ma.cd_futai
                          select m).FirstOrDefault();
        ...
        /// <returns>チェック結果</returns>
        private static bool ContainsKey(List<dynamic> keys, ma_futai entity)
        {
            return keys.Find(k => k.cd_futai == entity.cd_futai) != null;
        }
	}
}