				...
				while(k < datalen) {
					...
					switch (pos) {
						...
						case 'after':
							...
							if (sind) {
								if($(t.rows[sind.rowIndex+1]).hasClass("ui-subgrid")) { $(t.rows[sind.rowIndex+1]).after(row.join('')); }
		...
		NumberFormat : function(nData,opts) {
			...
			if($.fmatter.isNumber(nData)) {
				...
				if($.fmatter.isNumber(opts.decimalPlaces)) {
                    ...
                    // switch round or floor cliculate
					sOutput = Math.round(nData*nDecimal)/nDecimal + "";
                    if (sOutput > nData) {
                        // fix js multiplication error
                        sOutput = Math.floor(nData*nDecimal)/nDecimal + "";
                    }
