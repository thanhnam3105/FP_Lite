...
$.jgrid.extend({
			...
			if ( $("#"+IDs.themodal).html() != null ) {
				...
				for(i=0;i<this.p.colNames.length;i++){
				    if (!$t.p.colModel[i].hidedlg) { // added from T. Tomov
				        var colNameReplace = this.p.colNames[i].replace("<br>", " ");
				        if (App.ui.page.lang == "jp" || App.ui.page.lang == "zh") {
				            colNameReplace = this.p.colNames[i].replace("<br>", "");
				        }
						...
						((this.p.colModel[i].hidden === false) ? "checked" : "") + "/>" + "<label for='col_" + this.p.colModel[i].name + "'>" + colNameReplace + ((p.colnameview) ? " (" + this.p.colModel[i].name + ")" : "") + "</label></td></tr>";
				    }
				}
    ...
    ///<summary>auto resize height when resize with column</summary>
    ///<mode>funtions call resize<mode>
    ///<isReturn>language accept auto resize<isReturn>
    ///<grid>ID of grid<grid>
	resizeHeightThead: function (mode, isReturn, grid) {
	    //return when lang is not auto resize
	    if (!isReturn)
	        return;
	    //css auto resize heigth thead
	    $(".ui-jqgrid .ui-jqgrid-htable th div").addClass("html-jqgrid-resize-height");
	    var dtblGrid = (grid) ? $("#gbox_" + grid + " #gview_" + grid) : $("#gbox_item-grid #gview_item-grid");
	    //replace height thead grid
	    switch (mode) {
	        case "saveColumnData":
	        case "resizeStop":
	            var hei = dtblGrid.find(".ui-jqgrid-hdiv .ui-jqgrid-hbox").height();
	            dtblGrid.find(".frozen-div, .ui-th-column thed").css("height", hei);
	            dtblGrid.find(".frozen-bdiv").css("top", hei + 1);
	            dtblGrid.find(".frozen-div .ui-jqgrid-htable thead .ui-jqgrid-labels .ui-th-column").css("height", hei);
	            break;
	        case "gridComplete":
	            var hei = dtblGrid.find(".ui-jqgrid-hdiv .ui-jqgrid-hbox").height();
	            dtblGrid.find(".frozen-div .ui-jqgrid-htable thead .ui-jqgrid-labels .ui-th-column").css("height", hei);
