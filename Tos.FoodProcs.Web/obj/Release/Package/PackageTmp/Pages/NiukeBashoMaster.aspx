<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true"
    CodeBehind="NiukeBashoMaster.aspx.cs" Inherits="Tos.FoodProcs.Web.Pages.NiukeBashoMaster" %>

<asp:Content ID="Content1" ContentPlaceHolderID="IncludeContent" runat="server">
    <script src="<%=ResolveUrl("~/Resources/pages/pagedata-niukebashomaster." + System.Threading.Thread.CurrentThread.CurrentUICulture.TwoLetterISOLanguageName + ".js") %>"
        ...
        $(App.ui.page).on("ready", function () {
            ...
            */
            ...
            grid.jqGrid({
                ...
                colNames: [
                    pageLangText.cd_niuke.text + pageLangText.requiredMark.text
                    , pageLangText.nm_niuke.text + pageLangText.requiredMark.text
                    , pageLangText.kbn_niuke_basho.text + pageLangText.requiredMark.text
                    , pageLangText.jusho_1.text
                    , pageLangText.jusho_2.text
                    , pageLangText.jusho_3.text
                ...
                colModel: [
                    { name: 'cd_niuke_basho', width: 95, hidden: false, editable: true, sorttype: "text" },
                    { name: 'nm_niuke', width: 290, hidden: false, editable: true, sorttype: "text" },
                    { name: 'kbn_niuke_basho', width: 130, sortable: true, editable: true, resizable: false, hidden: false, hidedlg: false, edittype: 'select',
                        editoptions: {
                            value: function () {
                                // グリッド内のドロップダウンの生成
                                return app_util.prototype.makeJQGridDropDownStr(grid, kbnNiukeList, "nm_kbn_niuke", "kbn_niuke_basho", true);
                            },
                    ...
                    { name: 'nm_jusho_1', width: 215, hidden: false, editable: true, sorttype: "text" },
                    { name: 'nm_jusho_2', width: 215, hidden: false, editable: true, sorttype: "text" },
                    { name: 'nm_jusho_3', width: 215, hidden: false, editable: true, sorttype: "text" },
                    { name: 'flg_mishiyo', width: 95, editable: true, hidden: false, edittype: 'checkbox',
                ...
                loadComplete: function () {
                    ...
                    for (var i = 0; i < ids.length; i++) {
                        ...
                        if (grid.jqGrid('getCell', id, 'ts')) {
                            grid.jqGrid('setCell', id, 'cd_niuke_basho', '', 'not-editable-cell');
                        }
                    }
                    // グリッドの先頭行選択
            ...
            App.deferred.parallel({
                ...
                //             TODO: ここまで
                kbnNiuke: App.ajax.webget("../Services/FoodProcsService.svc/ma_kbn_niuke?$filter=flg_mishiyo eq " + pageLangText.shiyoMishiyoFlg.text + "&$orderby=kbn_niuke_basho")
            }).done(function (result) {
                kbnNiukeList = result.successes.kbnNiuke.d;
            ...
            var setCreatedChangeData = function (newRow) {
                // TODO: 画面の仕様に応じて以下の項目を変更してください。
                var changeData = {
                    "cd_niuke_basho": newRow.cd_niuke_basho,
                    "nm_niuke": newRow.nm_niuke,
                    "kbn_niuke_basho": newRow.kbn_niuke_basho,
                    "nm_jusho_1": newRow.nm_jusho_1,
                    "nm_jusho_2": newRow.nm_jusho_2,
                    "nm_jusho_3": newRow.nm_jusho_3,
            ...
            var setUpdatedChangeData = function (row) {
                // TODO: 画面の仕様に応じて以下の項目を変更してください。
                var changeData = {
                    "cd_niuke_basho": row.cd_niuke_basho,
                    "nm_niuke": row.nm_niuke,
                    "kbn_niuke_basho": row.kbn_niuke_basho,
                    "nm_jusho_1": row.nm_jusho_1,
                    "nm_jusho_2": row.nm_jusho_2,
                    "nm_jusho_3": row.nm_jusho_3,
                    ...
                    for (var i = 0; i < ret.length; i++) {
                        ...
                        if (ret[i].InvalidationName === "DuplicateKey") {
                            for (var j = 0; j < ids.length; j++) {
                                ...
                                retValue = ret[i].Data.cd_niuke_basho;
                                // TODO: ここまで
                                ...
                                if (value === retValue) {
                                    ...
                                    // 対象セルの背景変更
                                    grid.setCell(ids[j], firstCol, retValue, { background: '#ff6666' });
                                }
                                // TODO: ここまで
                            }
                        }
                            ...
                            for (p in changeSet.changeSet.deleted) {
                                ...
                                // TODO: 画面の仕様に応じて以下の値を変更します。
                                value = changeSet.changeSet.deleted[p].cd_niuke_basho;
                                retValue = ret[i].Data.cd_niuke_basho;
                                // TODO: ここまで
                ...
                if (!App.isUndefOrNull(ret.Updated) && ret.Updated.length > 0) {
                    for (var i = 0; i < ret.Updated.length; i++) {
                        for (p in changeSet.changeSet.updated) {
                            ...
                            retValue = ret.Updated[i].Requested.cd_niuke_basho;
                            // TODO: ここまで

                            if (value === retValue) {
                                // 更新状態の変更セットから変更データを削除
                                changeSet.removeUpdated(p);
                                var upCurret = ret.Updated[i].Current;

                                // 他のユーザーによって削除されていた場合
                                if (App.isUndefOrNull(upCurret)) {
                                    // 対象行の削除
                                    grid.delRowData(p);
                                    // メッセージの表示
                                    App.ui.page.notifyAlert.message(
                                    ...
                                    current.cd_niuke_basho = upCurret.cd_niuke_basho;
                                    current.nm_niuke = upCurret.nm_niuke;
                                    current.kbn_niuke_basho = upCurret.kbn_niuke_basho;
                                    current.nm_jusho_1 = upCurret.nm_jusho_1;
                                    current.nm_jusho_2 = upCurret.nm_jusho_2;
                                    current.nm_jusho_3 = upCurret.nm_jusho_3;
                                    current.flg_mishiyo = upCurret.flg_mishiyo;
                                    current.ts = upCurret.ts;
                                    // TODO: ここまで
                ...
                if (!App.isUndefOrNull(ret.Deleted) && ret.Deleted.length > 0) {
                    for (var i = 0; i < ret.Deleted.length; i++) {
                        for (p in changeSet.changeSet.deleted) {
                            ...
                            // TODO: 画面の仕様に応じて以下の値を変更します。
                            value = changeSet.changeSet.deleted[p].cd_niuke_basho;
                            retValue = ret.Deleted[i].Requested.cd_niuke_basho;
                            // TODO: ここまで
            ...
            var checkSave = function () {
                ...
                if (App.isUndefOrNull(selectedRowId)) {
                    ...
                    if (noChange()) {
                        ...
                        var resultValidateTable = validateAllTable();
                        var resultValidateChangeSet = validateChangeSet();
                        if (resultValidateTable == false || resultValidateChangeSet == false) {
                            App.ui.loading.close();
                            return;
                        }
                        else {
                            // チェックがすべて終了してからローディングを終了する
                            App.ui.loading.close();
                            // 保存確認ダイアログ
            ...
            var saveData = function (e) {
                ...
                // TODO: 画面の仕様に応じて以下のサービスのエンドポイントを変更してください。
                var saveUrl = "../api/NiukeBashoMaster";
                // TODO: ここまで
                App.ajax.webpost(
                    saveUrl, getPostData()
                ).done(function (result) {
            ...
            var validateCell = function (selectedRowId, cellName, value, iCol, isShowMess) {
            ...
            /// <param name="selectedRowId">選択行ID</param>
            var validateRow = function (selectedRowId, filter, isShowMess) {
                ...
                for (var i = 1; i < colModel.length; i++) {
                    ...
                    // セルバリデーション
                    if (App.isUndefOrNull(filter) || colModel[i].name != filter) {
                        if (!validateCell(selectedRowId, colModel[i].name, grid.getCell(selectedRowId, colModel[i].name), i, isShowMess)) {
                            if (colModel[i].name == "kbn_niuke_basho") {
                                idFirstRowKbnNiukeErr = idFirstRowKbnNiukeErr == "" ? selectedRowId : idFirstRowKbnNiukeErr;
                            }
                            isValid = false;
                        }
                    }
            ...
            var validateChangeSet = function () {
                for (p in changeSet.changeSet.created) {
                    ...
                    // カレントの行バリデーションを実行
                    if (!validateRow(p, "kbn_niuke_basho")) {
                        return false;
                    }
                }
                for (p in changeSet.changeSet.updated) {
            ...
            var validateAllTable = function () {
                ...
                for (var i = 0; i < ids.length; i++) {
                    var id = ids[i];
                    if (!validateRow(id, null, false)) {
                        result = false;
                    };
                }

                if (idFirstRowKbnNiukeErr != "") {
                    validateRow(idFirstRowKbnNiukeErr, null, true)
                }

                return result;
            }

            var isValidkbnNiukeBasho = function (kbnNiuke) {
                var isValid = true;
                var result = "";
                if (kbnNiuke != "" || kbnNiuke != null) {
                    $.each(kbnNiukeList, function (index, item) {
                        if (item.kbn_niuke_basho == kbnNiuke) {
                            result = kbnNiuke;
                        }
                    })
                    if (result == "") {
            ...
            validationSetting.kbn_niuke_basho.rules.custom = function (value) {
                return isValidkbnNiukeBasho(value);
            }
...
<asp:Content ID="Content3" ContentPlaceHolderID="MainContent" runat="server">
    <!-- 画面デザイン -- Start -->
    <!-- 検索条件と検索ボタン -->
    <div class="content-part search-criteria" style="display: none;">
    </div>
    <!-- 検索結果一覧 -->
