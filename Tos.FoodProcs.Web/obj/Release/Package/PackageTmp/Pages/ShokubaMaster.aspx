<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true"
    CodeBehind="ShokubaMaster.aspx.cs" Inherits="Tos.FoodProcs.Web.Pages.ShokubaMaster" %>

<asp:Content ID="Content1" ContentPlaceHolderID="IncludeContent" runat="server">
    <script src="<%=ResolveUrl("~/Resources/pages/pagedata-shokubamaster." + System.Threading.Thread.CurrentThread.CurrentUICulture.TwoLetterISOLanguageName + ".js") %>"
        ...
        $(App.ui.page).on("ready", function () {
            ...
            */
            ...
            grid.jqGrid({
                ...
                colNames: [
                    pageLangText.shokubaCode.text + pageLangText.requiredMark.text
                    , pageLangText.shokubaName.text + pageLangText.requiredMark.text
                ...
                colModel: [
                    { name: 'cd_shokuba', width: 100, editable: true, hidden: false },
                    { name: 'nm_shokuba', width: 400, editable: true, sorttype: "text" },
                ...
                loadComplete: function () {
                    ...
                    for (var i = 0; i < ids.length; i++) {
                        ...
                        if (grid.jqGrid('getCell', id, 'ts')) {
                            grid.jqGrid('setCell', id, 'cd_shokuba', '', 'not-editable-cell');
                        }
                    }
                    // グリッドの先頭行選択
                    ...
                    for (var i = 0; i < ret.length; i++) {
                        ...
                        if (ret[i].InvalidationName == "NotExsists") {
                            ...
                            for (var j = 0; j < ids.length; j++) {
                                ...
                                retValue = ret[i].Data.cd_shokuba;
                                // TODO: ここまで
                                ...
                                if (value === retValue) {
                                    ...
                                    // 対象セルの背景変更
                                    grid.setCell(ids[j], firstCol, ret[i].Data.cd_shokuba, { background: '#ff6666' });
                                    // TODO: ここまで
                                }
                            }
                        }
                        if (ret[i].InvalidationName == "DuplicateKey") {
                            ...
                            for (p in changeSet.changeSet.deleted) {
                                ...
                                // TODO: 画面の仕様に応じて以下の値を変更します。
                                value = changeSet.changeSet.deleted[p].cd_shokuba;
                                retValue = ret[i].Data.cd_shokuba;
                                // TODO: ここまで
                ...
                if (!App.isUndefOrNull(ret.Updated) && ret.Updated.length > 0) {
                    for (var i = 0; i < ret.Updated.length; i++) {
                        for (p in changeSet.changeSet.updated) {
                            ...
                            retValue = ret.Updated[i].Requested.cd_shokuba;
                            // TODO: ここまで
                            ...
                            if (value === retValue) {
                                ...
                                if (App.isUndefOrNull(upCurrent)) {
                                    ...
                                    current.cd_shokuba = upCurrent.cd_shokuba;
                                    current.nm_shokuba = upCurrent.nm_shokuba;
                ...
                if (!App.isUndefOrNull(ret.Deleted) && ret.Deleted.length > 0) {
                    for (var i = 0; i < ret.Deleted.length; i++) {
                        for (p in changeSet.changeSet.deleted) {
                            ...
                            // TODO: 画面の仕様に応じて以下の値を変更します。
                            value = changeSet.changeSet.deleted[p].cd_shokuba;
                            retValue = ret.Deleted[i].Requested.cd_shokuba;
                            // TODO: ここまで
