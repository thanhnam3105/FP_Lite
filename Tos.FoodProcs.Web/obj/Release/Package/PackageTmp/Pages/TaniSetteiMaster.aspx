<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true"
    CodeBehind="TaniSetteiMaster.aspx.cs" Inherits="Tos.FoodProcs.Web.Pages.TaniSetteiMaster" %>

<asp:Content ID="Content1" ContentPlaceHolderID="IncludeContent" runat="server">
    <script src="<%=ResolveUrl("~/Resources/pages/pagedata-tanisetteimaster." + System.Threading.Thread.CurrentThread.CurrentUICulture.TwoLetterISOLanguageName + ".js") %>"
        ...
        $(App.ui.page).on("ready", function () {
            ...
            */
            ...
            grid.jqGrid({
                ...
                // 列モデルの定義
                colModel: [
                    { name: 'cd_tani', width: 100, hidden: true, hidedlg: true, editable: false, sorttype: "text" },
                    { name: 'nm_tani', width: 500, hidden: false, editable: false, sorttype: "text" },
                    { name: 'flg_kinshi', width: 80, hidden: true, hidedlg: true, editable: false, sorttype: "text" },
                    { name: 'flg_mishiyo', width: 95, hidden: false, editable: false, edittype: 'checkbox',
                ...
                loadComplete: function () {
                    ...
                    for (var i = 0; i < ids.length; i++) {
                        var id = ids[i];
                        var inputFlg = grid.getRowData(id).flg_kinshi;
                        // TODO：画面の仕様に応じて以下の操作可否の定義を変更してください。
                        if (inputFlg == 1) {
                            //grid.jqGrid('setCell', id, 'flg_mishiyo', '', 'not-editable-cell');
                            $("#" + (id) + " td:eq(4) input").attr("disabled", true);
                        }
                        // TODO：ここまで
                    }
                    // グリッドの先頭行選択
            ...
            var query = function () {
                var query = {
                    url: "../Services/FoodProcsService.svc/ma_tani",
                    filter: createFilter(),
                    orderby: "cd_tani",
            ...
            var createFilter = function () {
                ...
                if (!App.isUndefOrNull(criteria.cd_tani) && criteria.cd_tani.length > 0) {
                    filters.push("cd_tani eq " + criteria.cd_tani);
                }
                if (!App.isUndefOrNull(criteria.nm_tani) && criteria.nm_tani.length > 0) {
                    filters.push("nm_tani eq " + criteria.nm_tani);
                }
                        ...
                        for (var j = 0; j < ids.length; j++) {
                            ...
                            if (value === retValue) {
                                ...
                                if (ret[i].ColumnName === "keys") {
                                    ...
                                    // 対象セルの背景変更
                                    grid.setCell(ids[j], firstCol, ret[i].Data.cd_tani, { background: '#ff6666' });
                                }
                                else {
                                    // エラーメッセージの表示
                                    App.ui.page.notifyAlert.message(
                ...
                if (!App.isUndefOrNull(ret.Updated) && ret.Updated.length > 0) {
                    for (var i = 0; i < ret.Updated.length; i++) {
                        for (p in changeSet.changeSet.updated) {
                            ...
                            retValue = ret.Updated[i].Requested.cd_tani;
                            // TODO: ここまで
                            ...
                            if (value === retValue) {
                                ...
                                if (App.isUndefOrNull(upCurrent)) {
                                    ...
                                    current.cd_tani = upCurrent.cd_tani;
                                    current.nm_tani = upCurrent.nm_tani;
            ...
            var saveData = function (e) {
                ...
                var saveUrl = "../api/TaniSetteiMaster";
                App.ajax.webpost(
                    saveUrl, getPostData()
                ).done(function (result) {
                    // 検索前の状態に初期化
            ...
            /// <summary>保存確認ダイアログのアップロードボタンクリック時のイベント処理を行います。</summary>
            $(".save-confirm-dialog .dlg-yes-button").on("click", saveData);
            /// <summary>保存確認ダイアログのキャンセルボタンクリック時のイベント処理を行います。</summary>
