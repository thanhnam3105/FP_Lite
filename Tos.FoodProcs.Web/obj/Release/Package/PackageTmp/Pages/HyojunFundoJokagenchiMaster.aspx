<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true"
    CodeBehind="HyojunFundoJokagenchiMaster.aspx.cs" Inherits="Tos.FoodProcs.Web.Pages.HyojunFundoJokagenchiMaster" %>

<asp:Content ID="Content1" ContentPlaceHolderID="IncludeContent" runat="server">
    <script src="<%=ResolveUrl("~/Resources/pages/pagedata-hyojunfundojokagenchimaster." + System.Threading.Thread.CurrentThread.CurrentUICulture.TwoLetterISOLanguageName + ".js") %>"
    ...
    <script type="text/javascript">
        ...
        $(App.ui.page).on("ready", function () {
            ...
            */
            ...
            grid.jqGrid({
                colNames: [
                    pageLangText.cd_fundo.text + pageLangText.requiredMark.text
                    , pageLangText.wt_fundo.text
                    , pageLangText.wt_fundo_jogen.text
                    , pageLangText.wt_fundo_kagen.text
                    , pageLangText.flg_mishiyo.text
                    , pageLangText.ts.text
                ],
                colModel: [
                    { name: 'cd_fundo', width: pageLangText.cd_fundo_width.number, editable: true, sorttype: "text" },
                    { name: 'wt_fundo', width: 150, editable: true, align: "right", sorttype: "float",
                        formatter: 'number',
                        formatoptions: {
                            decimalSeparator: ".", thousandsSeparator: ",", decimalPlaces: 3, defaultValue: "0.000"
                        }
                    },
                    { name: 'wt_fundo_kagen', width: pageLangText.wt_fundo_kagen_width.number, editable: true, align: "right", sorttype: "float",
                        formatter: 'number',
                        formatoptions: {
                            decimalSeparator: ".", thousandsSeparator: ",", decimalPlaces: 3, defaultValue: "0.000"
                        }
                    },
                    { name: 'wt_fundo_jogen', width: pageLangText.wt_fundo_jogen_width.number, editable: true, align: "right", sorttype: "float",
                        formatter: 'number',
                        formatoptions: {
                            decimalSeparator: ".", thousandsSeparator: ",", decimalPlaces: 3, defaultValue: "0.000"
                        }
                ...
                loadComplete: function () {
                    ...
                    for (var i = 0; i < ids.length; i++) {
                        ...
                        if (grid.jqGrid('getCell', id, 'ts')) {
                            // 既存行のコードは編集不可にする
                            grid.jqGrid('setCell', id, 'cd_fundo', '', 'not-editable-cell');
                        }
                    }
            ...
            var query = function () {
                var query = {
                    url: "../Services/FoodProcsService.svc/ma_fundo",
                    //filter: createFilter(),
                    orderby: "cd_fundo",
            ...
            var setAddData = function () {
                var addData = {
                    "cd_fundo": "",
                    "wt_fundo": 0.000,
                    "wt_fundo_jogen": 0.000,
                    "wt_fundo_kagen": 0.000,
            ...
            var setCreatedChangeData = function (newRow) {
                var changeData = {
                    "cd_fundo": newRow.cd_fundo,
                    "wt_fundo": newRow.wt_fundo,
                    "wt_fundo_jogen": newRow.wt_fundo_jogen,
                    "wt_fundo_kagen": newRow.wt_fundo_kagen,
                    "cd_create": App.ui.page.user.Code,
                    "cd_update": App.ui.page.user.Code,
            ...
            var setUpdatedChangeData = function (row) {
                var changeData = {
                    "cd_fundo": row.cd_fundo,
                    "wt_fundo": row.wt_fundo,
                    "wt_fundo_jogen": row.wt_fundo_jogen,
                    "wt_fundo_kagen": row.wt_fundo_kagen,
                    "cd_update": App.ui.page.user.Code,
                    ...
                    for (var i = 0; i < ret.length; i++) {
                        if (ret[i].InvalidationName == "DuplicateKey") {
                            ...
                            for (var j = 0; j < ids.length; j++) {
                                value = grid.getCell(ids[j], fundoCol);
                                retValue = ret[i].Data.cd_fundo;

                                if (value == retValue) {
                                    unique = ids[j] + "_" + fundoCol;
                                    // エラー行を追加
                                    errRows.push(ids[j]);
                                    ...
                                    // 対象セルの背景変更
                                    grid.setCell(ids[j], fundoCol, retValue, { background: '#ff6666' });
                                }
                            }
                        }
                        else {
                            ...
                            for (p in changeSet.changeSet.deleted) {
                                ...
                                value = changeSet.changeSet.deleted[p].cd_fundo;
                                retValue = ret[i].Data.cd_fundo;
                                // TODO: ここまで
            ...
            var saveData = function (e) {
                ...
                var saveUrl = "../api/HyojunFundoJokagenchiMaster";
                App.ajax.webpost(
                    saveUrl, getPostData()
                ).done(function (result) {
                    // 検索前の状態に初期化
            ...
            var clearErrBgcorror = function (errIds) {
                for (var i = 0; i < errIds.length; i++) {
                    var id = errIds[i];
                    // 対象セルの背景リセット
                    grid.setCell(id, fundoCol, '', { background: 'none' });
                }
            };
            ...
            /// <summary>引数チェック：数値以外の場合、0を返す</summary>
            ...
            /// <summary>大小チェック：値1＞値2の場合flaseを返す</summary>
            /// <param name="valiObj">選択行ID</param>
            /// <param name="value1">値1：小</param>
            /// <param name="value2">値2：大</param>
            var isValidSizeCheck = function (valiObj, value1, value2) {
                // 引数チェック
                value1 = checkValue(value1);
                value2 = checkValue(value2);
                // メッセージの設定
                valiObj.messages.custom = pageLangText.errorSize.text;
            ...
            //// 標準分銅重量
            validationSetting.wt_fundo.rules.custom = function (value) {
                var selectedRowId = getSelectedRowId(true);
                var valiObj = validationSetting.wt_fundo;
                var kagen = getThousandsSeparatorDel(grid.getCell(selectedRowId, "wt_fundo_kagen"));

                // チェック処理
                var isVali = isValidSizeCheck(valiObj, kagen, value);
                return isVali;
            };
            //// 上限
            //validationSetting.wt_fundo_jogen.rules.custom = function (value) {
            //    var selectedRowId = getSelectedRowId(true);
            //    var valiObj = validationSetting.wt_fundo_jogen;
            //    var jogen = getThousandsSeparatorDel(value);
            //    var kagen = getThousandsSeparatorDel(grid.getCell(selectedRowId, "wt_fundo_kagen"));
            //
            //    // チェック処理
            //    var isVali = isValidSizeCheck(valiObj, kagen, jogen);
            //    return isVali;
            //};
            //// 下限
            validationSetting.wt_fundo_kagen.rules.custom = function (value) {
                var selectedRowId = getSelectedRowId(true);
                var valiObj = validationSetting.wt_fundo_kagen;
                var kagen = getThousandsSeparatorDel(value);
                var fundo = getThousandsSeparatorDel(grid.getCell(selectedRowId, "wt_fundo"));

                // チェック処理：下限＜標準分銅であること
                isVali = isValidSizeCheck(valiObj, kagen, fundo);
                return isVali;
            };
