<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true"
    CodeBehind="NiukeBashoKubunMaster.aspx.cs" Inherits="Tos.FoodProcs.Web.Pages.NiukeBashoKubunMaster" %>

<asp:Content ID="Content1" ContentPlaceHolderID="IncludeContent" runat="server">
    <script src="<%=ResolveUrl("~/Resources/pages/pagedata-niukebashokubunmaster." + System.Threading.Thread.CurrentThread.CurrentUICulture.TwoLetterISOLanguageName + ".js") %>"
        ...
        $(App.ui.page).on("ready", function () {
            ...
            */
            ...
            var showSaveConfirmDialog = function () {
                ...
                //saveConfirmDialog.draggable({ containment: document.body, scroll: false });   // IEでしか動かない？一旦保留
            ...
            /// <summary>グリッド選択行時の動きを定義します。</summary>
            ...
            grid.jqGrid({
                colNames: [
                    ...
                    pageLangText.nm_kbn_niuke.text + pageLangText.requiredMark.text,
                    pageLangText.flg_niuke.text,
                    pageLangText.flg_henpin.text,
                    pageLangText.flg_shukko.text,
                ...
                colModel: [
                    { name: 'kbn_niuke_basho', width: pageLangText.kbn_niuke_basho_width.number, hidden: false, editable: true, sorttype: "text" },
                    { name: 'nm_kbn_niuke', width: pageLangText.nm_kbn_niuke_width.number, hidden: false, editable: true, sorttype: "text" },
                    { name: 'flg_niuke', width: pageLangText.flg_niuke_width.number, editable: true, hidden: false, edittype: 'checkbox',
                        editoptions: { value: "1:0" }, formatter: "checkbox", formatoptions: { disabled: false }, align: 'center'
                    },
                    { name: 'flg_henpin', width: pageLangText.flg_henpin_width.number, editable: true, hidden: false, edittype: 'checkbox',
                        editoptions: { value: "1:0" }, formatter: "checkbox", formatoptions: { disabled: false }, align: 'center'
                    },
                    { name: 'flg_shukko', width: pageLangText.flg_shukko_width.number, editable: true, hidden: false, edittype: 'checkbox',
                        editoptions: { value: "1:0" }, formatter: "checkbox", formatoptions: { disabled: false }, align: 'center'
                    },
                ...
                loadComplete: function () {
                    ...
                    for (var i = 0; i < ids.length; i++) {
                        ...
                        if (grid.jqGrid('getCell', id, 'ts')) {
                            grid.jqGrid('setCell', id, 'kbn_niuke_basho', '', 'not-editable-cell');
                        }
                    }
                    // グリッドの先頭行選択
                ...
                afterSaveCell: function (selectedRowId, cellName, value, iRow, iCol) {
                    ...
                    // 明細/未使用の設定処理
                    if (grid.getCell(selectedRowId, "flg_mishiyo") === pageLangText.mishiyoMishiyoFlg.text) {
                        // 明細/未使用が未チェックの場合

                        // 明細/未使用にチェックを入れる
                        grid.setCell(selectedRowId, "flg_mishiyo", pageLangText.mishiyoMishiyoFlg.text);
                        // 更新状態の変更セットに変更データを追加
                        changeSetUpdated(selectedRowId, "flg_mishiyo");
                    }
                }
            });
            ...
            $(function () {
                if (userRoles === pageLangText.operator.text || userRoles === pageLangText.viewer.text) {
            ...
            var query = function () {
                var query = {
                    // TODO: 画面の仕様に応じて以下のサービスのエンドポイントを変更してください。
                    url: "../Services/FoodProcsService.svc/ma_kbn_niuke",
                    // TODO: ここまで
                    filter: createFilter(),
                    // TODO: 画面の仕様に応じて以下のソート条件を変更してください。
                    orderby: "kbn_niuke_basho",
                    // TODO: ここまで
            ...
            var createFilter = function () {
                ...
                if (!App.isUndefOrNull(criteria.kbn_niuke_basho) && criteria.kbn_niuke_basho.length > 0) {
                    filters.push("kbn_niuke_basho eq " + criteria.kbn_niuke_basho);
                }
                if (!App.isUndefOrNull(criteria.nm_kbn_niuke) && criteria.nm_kbn_niuke.length > 0) {
                    filters.push("nm_kbn_niuke eq " + criteria.nm_kbn_niuke);
                }
                // TODO: ここまで
            ...
            var bindData = function (result) {
                ...
                if (parseInt(querySetting.count) > querySetting.top) {
                    App.ui.page.notifyAlert.message(
                        App.str.format(MS0681, querySetting.top)
                    ).show();
                }
            ...
            var setCreatedChangeData = function (newRow) {
                ...
                var changeData = {
                    ...
                    "nm_kbn_niuke": newRow.nm_kbn_niuke,
                    "flg_niuke": 0,
                    "flg_henpin": 0,
                    "flg_shukko": 0,
                    "flg_mishiyo": 0,
            ...
            var setUpdatedChangeData = function (row) {
                ...
                var changeData = {
                    ...
                    "nm_kbn_niuke": row.nm_kbn_niuke,
                    "flg_niuke": row.flg_niuke,
                    "flg_henpin": row.flg_henpin,
                    "flg_shukko": row.flg_shukko,
                    ...
                    for (var i = 0; i < ret.length; i++) {
                        ...
                        if (ret[i].InvalidationName == "DuplicateKey") {
                            ...
                            for (var j = 0; j < ids.length; j++) {
                                ...
                                retValue = ret[i].Data.kbn_niuke_basho;
                            ...
                            for (p in changeSet.changeSet.deleted) {
                                ...
                                // TODO: 画面の仕様に応じて以下の値を変更します。
                                value = changeSet.changeSet.deleted[p].kbn_niuke_basho;
                                retValue = ret[i].Data.kbn_niuke_basho;
                                // TODO: ここまで
...
//                                    changeSet.removeDeleted(p);
                ...
                if (!App.isUndefOrNull(ret.Updated) && ret.Updated.length > 0) {
                    for (var i = 0; i < ret.Updated.length; i++) {
                        for (p in changeSet.changeSet.updated) {
                            ...
                            retValue = ret.Updated[i].Requested.kbn_niuke_basho;
                            // TODO: ここまで
                            ...
                            if (value === retValue) {
                                ...
                                if (App.isUndefOrNull(upCurrent)) {
                                    ...
                                    current.kbn_niuke_basho = upCurrent.kbn_niuke_basho;
                                    current.nm_kbn_niuke = upCurrent.nm_kbn_niuke;
                                    current.flg_niuke = upCurrent.flg_niuke;
                                    current.flg_henpin = upCurrent.flg_henpin;
                                    current.flg_shukko = upCurrent.flg_shukko;
                ...
                if (!App.isUndefOrNull(ret.Deleted) && ret.Deleted.length > 0) {
                    for (var i = 0; i < ret.Deleted.length; i++) {
                        for (p in changeSet.changeSet.deleted) {
                            ...
                            // TODO: 画面の仕様に応じて以下の値を変更します。
                            value = changeSet.changeSet.deleted[p].kbn_niuke_basho;
                            retValue = ret.Deleted[i].Requested.kbn_niuke_basho;
                            // TODO: ここまで
            ...
            var saveData = function (e) {
                ...
                var saveUrl = "../api/NiukeBashoKubunMaster";
                App.ajax.webpost(
                    saveUrl, getPostData()
                ).done(function (result) {
                    // 検索前の状態に初期化
