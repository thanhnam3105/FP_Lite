<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="FutaiDialog.aspx.cs" Inherits="Tos.FoodProcs.Web.Pages.Dialog.FutaiDialog" %>
        ...
        $.dlg.register("FutaiDialog", {
            ...
            initialize: function (context) {
                ...
                dialog_grid.jqGrid({
                    ...
                    colNames: [
                        pageLangText.cd_futai_dlg.text
                        , pageLangText.nm_futai_dlg.text
                    ],
                    // todo：ここまで
                    // todo：画面の仕様に応じて以下の列モデルの定義を変更してください。
                    colModel: [
                        { name: 'cd_futai', width: 115, align: "left" },
                        { name: 'nm_futai', width: 360, align: "left" }
                    ],
                    // todo：ここまで
                    datatype: "local",
                    shrinktofit: false,
                ...
                var createFilterDialog = function () {
                    ...
                    if (!App.isUndefOrNull(criteria.nm_futai_dlg)) {
                        filters.push("substringof('" + encodeURIComponent(criteria.nm_futai_dlg) + "', cd_futai) "
                            + "or substringof('" + encodeURIComponent(criteria.nm_futai_dlg) + "', nm_futai)");
                    }
                    // TODO：ここまで
                ...
                var returnSelectedDialog = function () {
                    ...
                    for (var i = 0; i < selArray.length; i++) {
                        ...
                        selCode.push(row.cd_futai);
                        selName.push(row.nm_futai);
                    }
                    // TODO：ここまで
                ...
                <div class="dialog-search-criteria">
                    <div class="part-body" >
                        <ul class="item-list">
                            <li>
                                <label>
                                    <span class="item-label" data-app-text="nm_futai_dlg" style="width: 80px;"></span>
                                    <input type="text" id="con_nm_futai_dlg" name="nm_futai_dlg" style="width: 300px;" maxlength="50" />
                                </label>
                            </li>
                        </ul>
                    </div>
