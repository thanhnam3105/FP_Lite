<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="TorihikisakiDialog.aspx.cs" Inherits="Tos.FoodProcs.Web.Pages.Dialog.TorihikisakiDialog" %>
...
<head runat="server">
    ...
    <script type="text/javascript" >
        ...
        $.dlg.register("TorihikisakiDialog", {
            // TODO：ここまで
            initialize: function (context) {
                ...
                // パラメータ（取引先区分）を取得
                var torihikiKubun = context.data.param1;

                // ダイアログ内のグリッド定義
                dialog_grid.jqGrid({
                    colNames: [
                        pageLangText.torihikisakiCode.text
                        , pageLangText.torihikisakiName.text
                    ],
                    colModel: [
                        { name: 'cd_torihiki', width: 110, align: "left" },
                        { name: 'nm_torihiki', width: 350 }
                    ],
                    datatype: "local",
                    shrinkToFit: false,
                ...
                var queryDialog = function () {
                    var query = {
                        ...
                        orderby: "cd_torihiki",
                        select: "cd_torihiki, nm_torihiki",
                ...
                var createFilterDialog = function () {
                    ...
                    if (!App.isUndefOrNull(criteria.nm_torihikisaki) && criteria.nm_torihikisaki.length > 0) {
                        // 取引先コードor取引先名指定
                        //filters.push("substringof('" + encodeURIComponent(criteria.nm_torihikisaki) + "', nm_torihiki) eq true");
                        filters.push("(substringof('" + encodeURIComponent(criteria.nm_torihikisaki) + "', cd_torihiki) "
                            + "or substringof('" + encodeURIComponent(criteria.nm_torihikisaki) + "', nm_torihiki))");
                    }
                    if (!App.isUndefOrNull(torihikiKubun) && torihikiKubun.length > 0) {
                        // 取引先区分指定
                        filters.push("kbn_torihiki eq " + torihikiKubun);
                    }
                ...
                var displayCountDialog = function (resultCount) {
                ...
                var bindDataDialog = function (result) {
                    ...
                    displayCountDialog(resultCount);
                    // データバインド
                ...
                var returnSelectedDialog = function () {
                    ...
                    for (var i = 0; i < selArray.length; i++) {
                        ...
                        selCode.push(row.cd_torihiki);
                        selName.push(row.nm_torihiki);
                    }
                    // TODO：ここまで
    ...
    <div class="dialog-content">
        <!-- TODO: 画面の仕様に応じて以下のダイアログの設定を変更してください。 -->
            <div class="dialog-header">
                <h4 data-app-text="torihikisakiDialog"></h4>
            </div>
                ...
                <div class="dialog-search-criteria">
                    <div class="part-body" >
                        <ul class="item-list">
                            <li>
                                <label>
                                    <span class="item-label" data-app-text="torihikisakiName" style="width: 150px"></span>
                                    <input type="text" name="nm_torihikisaki" style="width: 200px;" maxlength="50" />
                                </label>
                            </li>
                        </ul>
                    </div>
