<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true"
    CodeBehind="GenkaCenterMaster.aspx.cs" Inherits="Tos.FoodProcs.Web.Pages.GenkaCenterMaster" %>

<asp:Content ID="Content1" ContentPlaceHolderID="IncludeContent" runat="server">
    <script src="<%=ResolveUrl("~/Resources/pages/pagedata-genkacentermaster." + System.Threading.Thread.CurrentThread.CurrentUICulture.TwoLetterISOLanguageName + ".js") %>"
        ...
        $(App.ui.page).on("ready", function () {
            ...
            */
            ...
            grid.jqGrid({
                ...
                colNames: [
                    pageLangText.genkaCode.text + pageLangText.requiredMark.text
                    , pageLangText.genkaName.text + pageLangText.requiredMark.text
                ...
                colModel: [
                    { name: 'cd_genka_center', width: 130, editable: true, hidden: false },
                    { name: 'nm_genka_center', width: 400, editable: true, sorttype: "text" },
            ...
            var query = function () {
                var query = {
                    url: "../Services/FoodProcsService.svc/ma_genka_center",
                    //filter: createFilter(),
                    orderby: "cd_genka_center",
            ...
            var bindData = function (result) {
                ...
                if (resultCount > querySetting.top) {
                    ...
                    App.ui.page.notifyInfo.message(App.str.format(pageLangText.overCount.text, querySetting.top)).show();
                }
                else {
                    ...
                    for (var i = 0; i < ret.length; i++) {
                        ...
                        if (ret[i].InvalidationName == "NotExsists") {
                            ...
                            for (var j = 0; j < ids.length; j++) {
                                ...
                                retValue = ret[i].Data.cd_genka_center;
                                // TODO: ここまで
                                ...
                                if (value === retValue) {
                                    ...
                                    // 対象セルの背景変更
                                    grid.setCell(ids[j], firstCol, ret[i].Data.cd_genka_center, { background: '#ff6666' });
                                    // TODO: ここまで
                                }
                            }
                        }
                        if (ret[i].InvalidationName == "DuplicateKey") {
                            ...
                            for (p in changeSet.changeSet.deleted) {
                                ...
                                // TODO: 画面の仕様に応じて以下の値を変更します。
                                value = changeSet.changeSet.deleted[p].cd_genka_center;
                                retValue = ret[i].Data.cd_genka_center;
                                // TODO: ここまで
                ...
                if (!App.isUndefOrNull(ret.Updated) && ret.Updated.length > 0) {
                    for (var i = 0; i < ret.Updated.length; i++) {
                        for (p in changeSet.changeSet.updated) {
                            ...
                            retValue = ret.Updated[i].Requested.cd_genka_center;
                            // TODO: ここまで
                ...
                if (!App.isUndefOrNull(ret.Deleted) && ret.Deleted.length > 0) {
                    for (var i = 0; i < ret.Deleted.length; i++) {
                        for (p in changeSet.changeSet.deleted) {
                            ...
                            // TODO: 画面の仕様に応じて以下の値を変更します。
                            value = changeSet.changeSet.deleted[p].cd_genka_center;
                            retValue = ret.Deleted[i].Requested.cd_genka_center;
                            // TODO: ここまで
            ...
            var saveData = function (e) {
                ...
                var saveUrl = "../api/GenkaCenterMaster";
                App.ajax.webpost(
                    saveUrl, getPostData()
                ).done(function (result) {
                    // 検索前の状態に初期化
