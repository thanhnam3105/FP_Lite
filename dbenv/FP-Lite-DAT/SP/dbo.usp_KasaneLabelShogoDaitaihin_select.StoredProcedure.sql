IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'usp_KasaneLabelShogoDaitaihin_select') AND type IN (N'P', N'PC')) 
DROP PROCEDURE [dbo].[usp_KasaneLabelShogoDaitaihin_select]
GO
SET ANSI_NULLS ON
GO
...
/*****************************************************
機能		：重ねラベル照合 代替品マスタ用
ファイル名	：usp_KasaneLabelShogoDaitaihin_select
...
作成日		：2016.09.09  BRC m.motojima	代替品対応
更新日		：2017.04.26  BRC d.kanehira    Q&Bサポート対応No.56
*****************************************************/
CREATE PROCEDURE [dbo].[usp_KasaneLabelShogoDaitaihin_select]
(
...
BEGIN
	SELECT
		...
		,hinKbnMa.nm_kbn_hin
	FROM tr_kowake tk
	INNER JOIN ma_daitaihin daitaihin
	ON tk.cd_hinmei = daitaihin.cd_hinmei
	AND daitaihin.cd_hinmei_daihyo = @cd_hinmei
	AND daitaihin.flg_mishiyo = @flg_mishiyo
	...
	WHERE
		...
		AND tk.cd_hinmei = daitaihin.cd_hinmei
