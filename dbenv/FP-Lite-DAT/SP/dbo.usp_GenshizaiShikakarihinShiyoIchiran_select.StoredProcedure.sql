IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'usp_GenshizaiShikakarihinShiyoIchiran_select') AND type IN (N'P', N'PC')) 
DROP PROCEDURE [dbo].[usp_GenshizaiShikakarihinShiyoIchiran_select]
GO
SET ANSI_NULLS ON
GO
...
-- Create date: <Create Date,,2014.06.18>
-- Last Update: <2018.01.30 motojima.m>
-- Description:	<Description,,原資材・仕掛品使用一覧> 
-- 検索条件/品区分で「原料」または「自家原料」が
-- 選択されたときの検索処理
--
-- ※※ 戻り値に修正がある場合 ※※
-- usp_GenshizaiShikakarihinShiyoIchiran_select_Resultは手で修正してください！
-- 一時テーブルを返却している為、関数インポートの「列情報の取得」では取得されません
-- ＃Resultがあれば実行できます。
-- =================================================================================
CREATE PROCEDURE [dbo].[usp_GenshizaiShikakarihinShiyoIchiran_select]
...
--		GROUP BY reci.cd_hinmei, reci.cd_haigo,reci.wt_shikomi, reci.no_han
	...
	ON recipe.cd_hinmei = genryo.cd_hinmei
...
--			GROUP BY reci.cd_hinmei, reci.cd_haigo,reci.wt_shikomi, reci.no_han
	...
	--ORDER BY genryo.cd_hinmei, recipe.cd_haigo, recipe.no_han , seihin.cd_hinmei
	...
	OR cd_shikakari1 IS NULL		-- または原料のみ（どこにも使用されていない原料）
